...
if ( nrow(DatasetInput) > 1 ) {
  for ( i in 2:nrow(DatasetInput) ) {
    ...
    targetFileTmp <- read.table(DatasetInput[i,"Target_file"], sep="\t", as.is=TRUE, header=TRUE)[,c(1:2)]
...
for (data_matrix in DatasetInput[ , "Expression_matrix" ] ) {
  ...
  if (!exists("datasets")) {
    datasets <- as.data.frame( read.table(gzfile(data_matrix), header=TRUE, sep="\t", row.names=NULL) )
    ...
    dataset <-as.data.frame( read.table(gzfile(data_matrix), header=TRUE, sep="\t", row.names=NULL) )
