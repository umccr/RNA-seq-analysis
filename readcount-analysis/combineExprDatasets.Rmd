...
targetFile <- read.table(DatasetInput[1,"Target_file"], sep="\t", as.is=TRUE, header=TRUE)[,c(1:2)]
...
if ( nrow(DatasetInput) > 1 ) {
  for ( i in 2:nrow(DatasetInput) ) {
    ...
    targetFile.list[[i]] <- read.table(DatasetInput[i,"Target_file"], sep="\t", as.is=TRUE, header=TRUE)[,c(1:2)]
...
write.table(prepare2write(datasets), file = paste0(params$projectDir, "/", paste(datasetIDs, collapse="_"),".combined_data_processed.txt"), sep="\t", quote=FALSE, row.names=FALSE, append = FALSE )

write.table(prepare2write(targetFile), file = paste0(params$projectDir, "/", paste(datasetIDs, collapse="_"),".combined_sample_annot.txt"), sep="\t", quote=FALSE, row.names=TRUE, append = FALSE )
```
...
keep <- rowSums(edgeR::cpm(y)>1) >= ncol(datasets)/10
...
y.cpm <- edgeR::cpm(y, normalized.lib.sizes=TRUE, log=TRUE, prior.count=0.25)
y.filtered.cpm <- edgeR::cpm(y.filtered, normalized.lib.sizes=TRUE, log=TRUE, prior.count=0.25)
```
...
y.filtered.norm.cpm <- edgeR::cpm(y.filtered.norm, normalized.lib.sizes=TRUE, log=TRUE, prior.count=0.25)
...
##### Generate scree plot
##### Prepare data frame
y.filtered.norm.cpm.subset_scree.df <- data.frame(paste0("PC ", c(1:length(importance_pca))), as.numeric(gsub("%", "",importance_pca)))
