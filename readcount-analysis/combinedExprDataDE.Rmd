...
```{r KEGG_over_representation_analysis, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 3, results = 'hide'}
  ...
  if ( !is.na(topGenes_list[[i]]) ) {
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      if ( !is.null(enrichKEGG.results) && nrow(enrichKEGG.results) > 0 ) {
        ...
        p1 <- barplot(top_enrichKEGG[[i]], orderBy = "p.adjust", decreasing = FALSE) + ylab("Count")
        p2 <- dotplot(top_enrichKEGG[[i]], orderBy = "p.adjust", decreasing = TRUE)
...
for( i in 1:ncol(eb$p.value) ){
  ...
  if ( length(topGenes_list[[i]]) > 1 ) {
    ...
    if ( !is.na(top_enrichKEGG[[i]]) ) {
      ...
      print(heatplot(top_enrichKEGG.genes[[i]], foldChange=data.pathways[[i]]))
      invisible(dev.off())
  ...
  if ( !is.na(topGenes_list[[i]]) ) {
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      if ( !is.null(gseKEGG.results) && nrow(gseKEGG.results) > 0 ) {
        ...
        print(dotplot(top_gseKEGG[[i]], orderBy = "p.adjust", decreasing = TRUE))
        invisible(dev.off())
      }
    }
  }
...
for( i in 1:ncol(eb$p.value) ){
  ...
  if ( length(topGenes_list[[i]]) > 1 ) {
    ...
    if ( !is.na(top_gseKEGG[[i]]) ) {
      ...
      print(heatplot(top_gseKEGG.genes[[i]], foldChange=data.pathways[[i]]))
      invisible(dev.off())
  ...
  if ( !is.na(topGenes_list[[i]]) ) {
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      if ( !is.null(enrichDGN.results) && nrow(enrichDGN.results) > 0 ) {
        ...
        p1 <- barplot(enrichDGN[[i]], orderBy = "p.adjust", decreasing = FALSE) + ylab("Count")
        p2 <- dotplot(enrichDGN[[i]], orderBy = "p.adjust", decreasing = FALSE)
        print(plot_grid(p1, p2, ncol=1, labels=LETTERS[1:2]))
        invisible(dev.off())
      }
    }
...
for( i in 1:ncol(eb$p.value) ){
  ...
  if ( length(topGenes_list[[i]]) > 1 ) {
    ...
    if ( !is.na(enrichDGN[[i]]) ) {
      ...
      print(heatplot(enrichDGN.genes[[i]], foldChange=data.pathways[[i]]))
      invisible(dev.off())
...
```{r DisGeNET_gsea_analysis, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 3, results = 'hide'}
##### Create lists to store pathways enrichemnt results for each comparison
top_gseNCG <- list()
top_gseNCG.df <- list()
  ...
  if ( !is.na(topGenes_list[[i]]) ) {
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      if ( !is.null(gseNCG.results) && nrow(gseNCG.results) > 0 ) {
        ...
        print(dotplot(top_gseNCG[[i]], orderBy = "p.adjust", decreasing = TRUE))
        invisible(dev.off())
      }
    }
  }
...
for( i in 1:ncol(eb$p.value) ){
  ...
  if ( length(topGenes_list[[i]]) > 1 ) {
    ...
    if ( !is.na(top_gseNCG[[i]]) ) {
      ...
      print(heatplot(top_gseNCG.genes[[i]], foldChange=data.pathways[[i]]))
      invisible(dev.off())
      ...
      for ( j in 1:nrow(top_gseNCG.df[[i]]) ) {
        ...
        if ( !is.null(gseNCG.results) && nrow(gseNCG.results) > 0 ) {
          png(file = paste(KEGG_dir, "/summary_", rownames(top_gseNCG.df[[i]])[j], ".enrichment_map.png", sep=""))
          print(emapplot(topPathways.genes, foldChange=data.pathways[[i]]))
          invisible(dev.off())
          
          cat(paste0("![](", paste(KEGG_dir, "/summary_", rownames(top_gseNCG.df[[i]])[j], ".enrichment_map.png)", sep="")), "\n")
          
        } else {
          cat(paste("\nEnrichment map for \"", top_gseNCG.df[[i]]$Description[j], "\" pathway is **not available** for ", contrastNames[i], " comparison\n", sep=""))
        }
        cat("\n***\n")
      }
