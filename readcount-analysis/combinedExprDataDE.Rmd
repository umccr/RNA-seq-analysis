...
The read count data is converted into **`r params$transform`**s using *[edgeR](https://bioconductor.org/packages/release/bioc/html/edgeR.html){target="_blank"}* functions. `r if ( !params$filter ) { c("The option for filtering out genes with low counts is switched OFF") } else if ( params$filter ) { paste0(before_filtering-after_filtering, " genes with low counts, which is **", filtered.percentage, "%** out of the total ",  before_filtering, " input genes) were filtered out. ") }`. `r if ( !params$log ) { c("The data is not log-transformed") } else if ( params$log ) { c("The data is log2-transformed") }`.
...
data.df$Transformed <- sort(data.df$Transformed)
data.df$Counts <- sort(data.df$Counts)
