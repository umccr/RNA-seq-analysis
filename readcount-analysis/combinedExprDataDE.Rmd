...
##### Function to show the created pathways PNG in the plot window and by default delete the file itself afterwards ( https://stackoverflow.com/questions/60141841/how-to-get-pathview-plot-displayed-directly-rather-than-saving-as-a-file-in-r )
...
```{r DE_heatmap, warning = FALSE, echo = TRUE, message = FALSE, results = 'asis', fig.width = 12, fig.height = 10}
...
```{r DE_heatmap_interactive, warning = FALSE, echo = TRUE, message = FALSE, results = 'asis', fig.width = 12, fig.height = 8}
if ( !is.null(topGenes.index) ) {
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  if ( topGenes_list[[i]] != 0 ) {
    topPathways_list[[i]] <- enrichKEGG(gene=topGenes_list[[i]]$ENTREZID, organism = 'hsa', pvalueCutoff = params$pThreshold, use_internal_data = TRUE)
...
Visualization KEGG pathways over-representation analysis results in individual groups comparisons. The gradient color scale illustrates expression log2 fold-change values observed in individual comparisons.
...
for( i in 1:ncol(eb$p.value) ){
  ...
  if ( topGenes_list[[i]] != 0 ) {
    ...
    if ( topPathways_list[[i]] != 0 ) {
      
      for ( j in 1:nrow(topPathways_list[[i]]) ) {
        
        ##### Prepare data with logFC values
        data.pathways <- topGenes_list[[i]]$log2FC
        names(data.pathways) <- topGenes_list[[i]]$ENTREZID
        
        ##### Present KEGG pathway with gene expression values mapped to gradient color scale
        cat(paste0("\n###### ", topPathways_list[[i]][j]$Description, "\n"))
        
        #see_pathview(gene.data = data.pathways, 
        #         pathway.id = topPathways_list[[i]][j]$ID,
        #         species = "hsa",  
        #         kegg.native = TRUE)
