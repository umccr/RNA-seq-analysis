...
params:
  ...
  adjMethod: "BH"
  pThreshold: 0.05
  pvalueCutoff_kegg: 0.05
  qvalueCutoff_kegg: 0.2
  pAdjustMethod_kegg: "BH"
  minGSSize_kegg: 5
  maxGSSize_kegg: 500
...
* **pvalueCutoff_kegg**: `r params$pvalueCutoff_kegg`
* **qvalueCutoff_kegg**: `r params$qvalueCutoff_kegg`
* **pAdjustMethod_kegg**: `r params$pAdjustMethod_kegg`
* **minGSSize_kegg**: `r params$minGSSize_kegg`
* **maxGSSize_kegg**: `r params$maxGSSize_kegg`
...
* p-value: **`r  params$pThreshold`**
* q-value: **0.5**
* minimal size of each geneSet: **120**

</font> 
</details>

```{r KEGG_over_representation_analysis, comment = NA, message=FALSE, warning=FALSE, results = 'hide'}
##### Create lists to store pathways enrichemnt results for each comparison
topPathways_list <- list()
topPathways_list_table <- list()
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  if ( !is.na(topGenes_list[[i]]) ) {
    ...
    ##### Also prepare reference for converting ENTREZIDs to gene symbols
    reference <- rownames(topGenes_list[[i]][ !is.na(topGenes_list[[i]]$ENTREZID),  ])
    names(reference) <- as.character(genes.ENTREZID)
        
    ##### Run analysis only for > 1 gene
    if ( length(genes.ENTREZID) > 1 ) {
      
      enrichKEGG.results <- enrichKEGG( gene=genes.ENTREZID, organism = 'hsa', pvalueCutoff = params$pvalueCutoff_kegg, pAdjustMethod = params$pAdjustMethod, qvalueCutoff = params$qvalueCutoff, minGSSize = params$minGSSize_kegg, maxGSSize = params$maxGSSize_kegg, use_internal_data = params$use_internal_data_kegg, universe = as.character(data.annot$ENTREZID[ !is.na(data.annot$ENTREZID) ]) )
      
      if ( !is.null(enrichKEGG.results) && nrow(enrichKEGG.results) > 0 ) {
        
        ##### Substitute ENTREZIDs with Gene symbols
        enrichKEGG.results4table <- as.data.frame(enrichKEGG.results)
        
        for ( j in 1: nrow(enrichKEGG.results) ) {
          enrichKEGG.results.genes <- unlist(strsplit(enrichKEGG.results4table$geneID[j], split='/', fixed=TRUE))
          
          enrichKEGG.results.genes_symbols<- ifelse(is.na(reference[enrichKEGG.results.genes]), enrichKEGG.results.genes, reference[enrichKEGG.results.genes])
          
          enrichKEGG.results4table$geneID[j] <- paste(enrichKEGG.results.genes_symbols, collapse = " / ")
        }
        
        topPathways_list[[i]] <- enrichKEGG.results4table[, c("Description", "GeneRatio", "pvalue", "p.adjust", "qvalue", "Count", "geneID")]
        topPathways_list_table[[i]]<- DT::datatable( data = as.data.frame(topPathways_list[[i]]), caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;'), filter = "top", extensions = c('Buttons','FixedColumns','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, fixedColumns = list(leftColumns = 1), deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE )
      } else {
        topPathways_list[[i]] <- NA
      }
    } else {
      topPathways_list[[i]] <- NA
    }
...
```{r KEGG_over_representation_visualisation, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, results="asis"}
##### Create folder for storing pathway analysis results
...
for( i in 1:ncol(eb$p.value) ){
  ...
  if ( !is.na(topGenes_list[[i]]) ) {
    ...
    if ( !is.na(topPathways_list[[i]]) ) {
      
      cat(paste("\n\nTable presenting summary of enriched KEGG pathways in ", contrastNames[i], " comparison.\n\n", sep=""))
      
      cat(renderTags(topPathways_list_table[[i]])$html)
      
      cat("\n***\n")
      cat(paste("Visualization of over-representated KEGG pathways. The gradient color scale illustrates expression log2 fold-change values observed in ", contrastNames[i], " comparison.\n\n", sep=""))
        
      for ( j in 1:nrow(topPathways_list[[i]]) ) {
        
        ##### Prepare data with logFC values
        ##### But first remove genes with invalid ENTREZIDs
        data.pathways <- topGenes_list[[i]]$log2FC[ !is.na(topGenes_list[[i]]$ENTREZID) ]
        data.pathways <- topGenes_list[[i]]$log2FC[ !is.na(topGenes_list[[i]]$ENTREZID) ]
        names(data.pathways) <- topGenes_list[[i]]$ENTREZID[ !is.na(topGenes_list[[i]]$ENTREZID) ]
        
        ##### Present KEGG pathway with gene expression values mapped to gradient color scale
        cat("\n###### ", topPathways_list[[i]]$Description[j], "\n")
        
        pathview.res <- pathview(gene.data  = data.pathways, pathway.id = rownames(topPathways_list[[i]])[j], species = "hsa", limit = list(gene=max(abs(data.pathways)), cpd=1), kegg.dir = KEGG_dir)
        
        cat(paste0("![](", paste(KEGG_dir, "/", rownames(topPathways_list[[i]])[j], ".pathview.png)", sep="")), "\n")
        cat("\n***\n")
      }
    } else {
