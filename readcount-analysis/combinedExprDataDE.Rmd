...
##### Define colour space ( https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html#colors )
col.h = colorRamp2(seq(min(data.h_batch_rm), max(data.h_batch_rm), length = 3), c("blue", "#EEEEEE", "red"))
  
#### Heatmap (AFTER batch correction)
Heatmap(data.h_batch_rm, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h)

pdf(paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_batch_rm.pdf")), width = 8, height = 8)
Heatmap(data.h_batch_rm, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h, use_raster = FALSE)
dev.off()
...
if ( length(topGenes.index) > 1 ) {
  ...
  print(Heatmap(data.h, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h))
...
```{r DE_heatmap_interactive, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, results = 'asis', fig.width = 12, fig.height = 10}
suppressMessages(library(heatmaply))
...
if ( length(topGenes.index) > 1 ) {
  output[[1]] <- heatmaply(data.frame(data.h), Rowv = hr, Colv = rev(hc), k_col=1, k_row=1, colors = colorRampPalette(c("darkblue","darkblue","darkslateblue","darkslateblue","white","firebrick3","firebrick3","firebrick4","firebrick4"))(100), col_side_colors = col_side_colors, col_side_palette = col_side_palette, scale="row", trace="none", hide_colorbar = TRUE, fontsize_row = 8, fontsize_col = 8) %>%
    layout(autosize = TRUE, width = 800, height = 600, margin = list(l=150, r=50, b=150, t=50, pad=4), showlegend = FALSE)
