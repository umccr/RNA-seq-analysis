...
params:
  exprDir: '/Users/jmarzec/data/Pancreatic_data_harmonization/expression/in-house/Avner_Birinapant/Combined_data'
  exprFile: 'Avner_Birinapant_Organoid.counts.matrix.txt'
  annotFile: 'Avner_Birinapant_Organoid_Target.txt'
  annotFeatures: 'Target,Subject,Timestamp'
  output_dir: '/Users/jmarzec/data/Pancreatic_data_harmonization/expression/in-house/Avner_Birinapant/Combined_data/Avner_Birinapant_Organoid_DE_adjP'
  transform: 'CPM'
  norm: 'TMM'
  batch_rm: FALSE
  filter: TRUE
  ...
  top_genes: 500
  ensembl: TRUE
  results_name: 'Avner_Birinapant_Organoid_DE'
  lfcThreshold: 2
  adjMethod: "BH"
  pThreshold: 0.05
  ref_data_dir: '../data'
...
if ( params$hide_code_btn ) {
  writeLines(".btn { display: none ;", con = "combinedExprDE.css")
} else {
  writeLines(" ", con = "combinedExprDE.css")
}
```
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  #####  P-value histograms
  png(file = paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_P_hist.png", sep=""), width = 6, height = 6, units = "in", res = 150)
  ...
  png(file = paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_volcano_plot", ".png", sep = ""), width = 6, height = 6, units = "in", res = 150)
...
```{r DE_overlap, comment = NA, message=FALSE, warning=FALSE, results = 'hide'}
eb.decideTests <- as.data.frame(decideTests(eb, method="separate", adjust.method=params$adjMethod, p.value=params$pThreshold, lfc=params$lfcThreshold))
summary(eb.decideTests)

#### Order the decideTests
eb.decideTests <- eb.decideTests[apply(eb.decideTests[,-1], 1, function(x) !all(x==0)),]
...
eb.decideTests <- eb.decideTests[ , names(sort(eb.decideTests.colSums, decreasing = TRUE)) ]
eb.decideTests <- eb.decideTests[ names(sort(eb.decideTests.rowSums, decreasing = TRUE)),  ]
...
for (i in 1:ncol(eb$p.value) ) {
	...
	topGenes <- topTable(eb, coef=colnames(eb)[i], adjust=params$adjMethod,  sort.by="p", p.value=params$pThreshold, lfc=params$lfcThreshold, number=nrow(DGEdata.voom))
  ...
  if ( length(topGenes) != 0 ) {
    ...
    write.table(topGenes, file=paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_DE_genes.txt", sep=""),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
    
    ##### Generate dataTable
    topGenes_list[[i]] <- DT::datatable( data = topGenes, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', htmltools::strong(colnames(eb$p.value)[i])), filter = "top", extensions = c('Buttons','FixedColumns','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, fixedColumns = list(leftColumns = 2), deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE ) %>%
        DT::formatStyle( columns = names(topGenes), 'text-align' = 'center' ) %>%
        DT::formatRound(columns = c("log2FC", "t-statistic"), 2)
    
    topGenes.index <- unique(c(topGenes.index,rownames(topGenes)))
...
```{r DE_heatmap_interactive, warning = FALSE, echo = TRUE, message = FALSE, results = 'hide', fig.width = 12, fig.height = 10}
suppressMessages(library(heatmaply))
...
htmltools::tagList(setNames(p, NULL))
...
`r if ( length(contrastNames) < 4 ) { c("#### Venn diagram") }`

```{r DE_venn, comment = NA, message=FALSE, warning=FALSE, results = 'hide'}
...
for( i in 1:ncol(eb$p.value) ){
  ...
  cat(renderTags(topGenes_list[[i]])$html)
