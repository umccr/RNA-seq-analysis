...
params:
  ...
  pAdjustMethod: "BH"
  eScoreThreshold: 0.2
  pThreshold_pathways: 0.001
  adjMethod_pathways: "none"
  poi: '/g/data3/gx8/projects/Jacek_Pancreatic_data_harmonization/expression/projects/PRMT5/Combined_data/pathways_of_interest.txt'
  minGSSize: 5
  maxGSSize: 500
...
* **eScoreThreshold**: `r params$eScoreThreshold`
* **pThreshold_pathways**: `r params$pThreshold_pathways`
* **adjMethod_pathways**: `r params$adjMethod_pathways`
* **poi**: `r params$poi`
...
* **run_kegg**: `r params$run_kegg`
* **kegg_plots**: `r params$kegg_plots`
* **use_internal_data_kegg**: `r params$use_internal_data_kegg`
* **run_DisGeNET**: `r params$run_DisGeNET`
* **run_reactome**: `r params$run_reactome`
* **top_genes**: `r params$top_genes`
* **split**: `r params$split`
...
##### Change "Metabolic" subtyping to "Karasinska"
annotFeatures <- gsub("Metabolic", "Karasinska", annotFeatures)
names(targetFile) <-gsub("Metabolic", "Karasinska", names(targetFile))

##### Define pathways of interest (if specified)
if ( params$poi != "none" ) {
  poi_list <- unique(read.table(params$poi, sep="\t", as.is=TRUE, header=FALSE, row.names=NULL, quote="")[,1])
  runPOI <- TRUE
} else {
  runPOI <- FALSE
}
...
cpm.min <- round(min(as.numeric(colSums(data)*1e-6)), digits=0)
cpm.max <- round(max(as.numeric(colSums(data)*1e-6)), digits=0)
cpm.min.sample <- colnames(data)[ order(colSums(data)*1e-6)[1] ]
cpm.max.sample <- colnames(data)[ order(colSums(data)*1e-6)[ncol(data)] ]
...
##### Calculate how many genes were filtered out from the data
if ( params$filter) {
  before_filtering <- nrow(data)
  after_filtering <- nrow(y$filtered.transformed)
  filtered.percentage <- round((before_filtering-after_filtering)*100/nrow(data), digits = 0)
}
```

The read count data is converted into **`r params$transform`**s using *[edgeR](https://bioconductor.org/packages/release/bioc/html/edgeR.html){target="_blank"}* functions. `r if ( !params$filter ) { c("The option for filtering out genes with low counts is switched OFF") } else if ( params$filter ) { paste0("**", before_filtering-after_filtering, "** genes with low counts, which is **", filtered.percentage, "%** out of the ",  before_filtering, " input genes, were filtered out") }`. `r if ( !params$log ) { c("The data is not log-transformed") } else if ( params$log ) { c("The data is log2-transformed") }`.

`r if ( params$transform == "CPM" ) { paste0("The CPM of 1 (cut-off for removing low expressed genes) corresponds to ", cpm.min, " reads in sample with the lowest sequencing depth (", cpm.min.sample, "), and ", cpm.max, " reads in sample with the greatest sequencing depth (", cpm.max.sample, "). The plot below presents the relation between read counts and the corresponding ", params$transform, " values in the sample with the lowest sequencing depth (", cpm.min.sample, "). The red vertical line indicates the threshold for filtering genes with low counts.") } else { paste0("The plot below presents the relation between read counts and the corresponding ", params$transform, " values in the patient data. The red vertical line indicates the threshold for filtering genes with low counts.") }`

```{r data_transformation_vs_counts_plot, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 6}
suppressMessages(library(plotly))

##### Organise the data into data frame
if ( params$log ) {
  data.df <- as.data.frame(cbind( exp(y$transformed[,cpm.min.sample]), data[, cpm.min.sample]))
  names(data.df) <- c("Transformed", "Counts")
  data.df$Transformed <- log(data.df$Transformed)

} else {
  data.df <- as.data.frame(cbind( y$transformed[,cpm.min.sample], data[, cpm.min.sample]))
  names(data.df) <- c("Transformed", "Counts")
  data.df$Transformed <- log(data.df$Transformed)
}
...
data.df <- data.df[ order(data.df$Transformed), ]

##### Generate plot for filtered data
counts_vs_transformed <- plot_ly( data.df, x = ~Transformed, y = ~Counts, width = 800, height = 300, color = I('black'), marker = list(size = 5), type="scatter", mode = "markers", name = paste0(params$transform, " / Counts (", cpm.min.sample, ")") ) %>% 
...
saveWidgetFix(counts_vs_transformed, file = paste(params$output_dir, "counts_vs_transformed.html", sep = "/"))

##### Detach plotly package. Otherwise it clashes with other graphics devices
detach("package:plotly", unload=FALSE)
```

Plot(s) below present `r params$transform` data distribution before and after filtering genes with low counts.
...
##### Add colours for Bailey subtypes (if provided)
HeatmapAnnotation.col.list <- "Target"
if ( "Bailey" %in% annotFeatures ) {
  Target.col <- c(Target.col, c("ADEX" = "darkorchid", "Immunogenic" = "firebrick2", "Squamous" = "goldenrod1", "Progenitor" = "dodgerblue3", "Unknown" = "gray"))
  HeatmapAnnotation.col.list <- c(HeatmapAnnotation.col.list, "Bailey")
}

##### Add colours for Collisson subtypes (if provided)
if ( "Collisson" %in% annotFeatures ) {
  Target.col <- c(Target.col, c("Exocrine-like" = "darkorchid", "Quasimesenchymal" = "goldenrod1", "Classical" = "dodgerblue3", "Unknown" = "gray"))
  HeatmapAnnotation.col.list <- c(HeatmapAnnotation.col.list, "Collisson")
}

##### Add colours for Moffitt subtypes (if provided)
if ( "Moffitt" %in% annotFeatures ) {
  Target.col <- c(Target.col, c("Basal-like" = "goldenrod1", "Classical" = "dodgerblue3", "Unknown" = "gray"))
  HeatmapAnnotation.col.list <- c(HeatmapAnnotation.col.list, "Moffitt")
}

##### Add colours for Moffitt_ss (single subject) subtypes (if provided)
if ( "Moffitt_ss" %in% annotFeatures ) {
  Target.col <- c(Target.col, c("Basal-like" = "goldenrod1", "Classical" = "dodgerblue3", "Unknown" = "gray"))
  HeatmapAnnotation.col.list <- c(HeatmapAnnotation.col.list, "Moffitt_ss")
}

##### Add colours for PuRIST subtypes (if provided)
if ( "PuRIST" %in% annotFeatures ) {
  Target.col <- c(Target.col, c("Basal-like" = "goldenrod1", "Classical" = "dodgerblue3", "Unknown" = "gray"))
  HeatmapAnnotation.col.list <- c(HeatmapAnnotation.col.list, "PuRIST")
}

##### Add colours for PuRIST subtypes (if provided)
if ( "PAMG" %in% annotFeatures ) {
  Target.col <- c(Target.col, c("Basal-like" = "goldenrod1", "Classical" = "dodgerblue3", "Mixed" = "gray"))
  HeatmapAnnotation.col.list <- c(HeatmapAnnotation.col.list, "PAMG")
}

##### Add colours for PuRIST subtypes (if provided)
if ( "Karasinska" %in% annotFeatures ) {
  Target.col <- c(Target.col, c("Quiescent" = "darkgoldenrod3", "Glycolytic" = "darkslategray", "Cholesterogenic" = "darkolivegreen3", "Mixed" = "tan4"))
  HeatmapAnnotation.col.list <- c(HeatmapAnnotation.col.list, "Karasinska")
}

##### Add colours for PuRIST scores (if provided)
if ( all(c("PuRIST_score","PAMG_score","EMT_score") %in% annotFeatures) ) {
  PuRIST_score.col <- colorRamp2(sort(targetFile$PuRIST_score), colorspace::diverge_hcl(n=length(targetFile$PuRIST_score), h=c(246, 40), c=96))
  
  PAMG_score.col <- colorRamp2(sort(targetFile$PAMG_score), colorspace::diverge_hcl(n=length(targetFile$PAMG_score), h=c(40, 246), c=96))
  
  EMT_score.col <- colorRamp2(sort(targetFile$EMT_score), colorspace::diverge_hcl(n=length(targetFile$EMT_score), h=c(246, 40), c=96))
  
  ha = HeatmapAnnotation(df = targetFile[, colnames(targetFile) %in% annotFeatures, drop = FALSE], col = eval(parse(text = paste0("list(", paste0(HeatmapAnnotation.col.list, "=Target.col", collapse=","), ", PuRIST_score=PuRIST_score.col, PAMG_score=PAMG_score.col, EMT_score=EMT_score.col)"))))
  
} else if ( all(c("PuRIST_score","PAMG_score") %in% annotFeatures) ) {
  PuRIST_score.col <- colorRamp2(sort(targetFile$PuRIST_score), colorspace::diverge_hcl(n=length(targetFile$PuRIST_score), h=c(246, 40), c=96))
  
  PAMG_score.col <- colorRamp2(sort(targetFile$PAMG_score), colorspace::diverge_hcl(n=length(targetFile$PAMG_score), h=c(40, 246), c=96))
  
  ha = HeatmapAnnotation(df = targetFile[, colnames(targetFile) %in% annotFeatures, drop = FALSE], col = eval(parse(text = paste0("list(", paste0(HeatmapAnnotation.col.list, "=Target.col", collapse=","), ", PuRIST_score=PuRIST_score.col, PAMG_score=PAMG_score.col)"))))
  
} else if ( all(c("PuRIST_score","EMT_score") %in% annotFeatures) ) {
  PuRIST_score.col <- colorRamp2(sort(targetFile$PuRIST_score), colorspace::diverge_hcl(n=length(targetFile$PuRIST_score), h=c(246, 40), c=96))
  
  EMT_score.col <- colorRamp2(sort(targetFile$EMT_score), colorspace::diverge_hcl(n=length(targetFile$EMT_score), h=c(246, 40), c=96))
  
  ha = HeatmapAnnotation(df = targetFile[, colnames(targetFile) %in% annotFeatures, drop = FALSE], col = eval(parse(text = paste0("list(", paste0(HeatmapAnnotation.col.list, "=Target.col", collapse=","), ", PuRIST_score=PuRIST_score.col, EMT_score=EMT_score.col)"))))
  
} else if ( "PuRIST_score" %in% annotFeatures ) {
  PuRIST_score.col <- colorRamp2(sort(targetFile$PuRIST_score), colorspace::diverge_hcl(n=length(targetFile$PuRIST_score), h=c(246, 40), c=96))
  
  ha = HeatmapAnnotation(df = targetFile[, colnames(targetFile) %in% annotFeatures, drop = FALSE], col = eval(parse(text = paste0("list(", paste0(HeatmapAnnotation.col.list, "=Target.col", collapse=","), ", PuRIST_score=PuRIST_score.col)"))))
  
} else if ( all(c("PAMG_score","EMT_score") %in% annotFeatures) ) {
  PAMG_score.col <- colorRamp2(sort(targetFile$PAMG_score), colorspace::diverge_hcl(n=length(targetFile$PAMG_score), h=c(40, 246), c=96))
  
  EMT_score.col <- colorRamp2(sort(targetFile$EMT_score), colorspace::diverge_hcl(n=length(targetFile$EMT_score), h=c(246, 40), c=96))
  
  ha = HeatmapAnnotation(df = targetFile[, colnames(targetFile) %in% annotFeatures, drop = FALSE], col = eval(parse(text = paste0("list(", paste0(HeatmapAnnotation.col.list, "=Target.col", collapse=","), ", PAMG_score=PAMG_score.col, EMT_score=EMT_score.col)"))))
  
} else if ( "PAMG_score" %in% annotFeatures ) {
  PAMG_score.col <- colorRamp2(sort(targetFile$PAMG_score), colorspace::diverge_hcl(n=length(targetFile$PAMG_score), h=c(40, 246), c=96))
  
  ha = HeatmapAnnotation(df = targetFile[, colnames(targetFile) %in% annotFeatures, drop = FALSE], col = eval(parse(text = paste0("list(", paste0(HeatmapAnnotation.col.list, "=Target.col", collapse=","), ", PAMG_score=PAMG_score.col)"))))
  
} else {
  ha = HeatmapAnnotation(df = targetFile[, colnames(targetFile) %in% annotFeatures, drop = FALSE], col = eval(parse(text = paste0("list(", paste0(HeatmapAnnotation.col.list, "=Target.col", collapse=","), ")"))))
}
...
if ( params$split && length(unique(targetFile$Target)) > 1 ) {
...
pdf(paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap.pdf")), width = 12, height = 12)
...
##### Cluster samples
data.h.z.heatmap <- data.h.z[, hc$labels[hc$order]]

##### Cluster genes
data.h.z.heatmap <- data.h.z.heatmap[hr$labels[hr$order], ]

##### Write data from the heatmap into a file
write.table(data.h.z.heatmap, paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_data.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
```
...
if ( length(unique(batches)) > 1 ) {
  ...
  data_before_batch_rm_heatmap <- data.h
...
##### Subset genes
data.h_batch_rm <- data.frame(data.h_batch_rm[sel,])
...
Heatmap(data.h_batch_rm.z, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h, column_title = "After batch correction")

pdf(paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_batch_rm.pdf")), width = 12, height = 12)
Heatmap(data.h_batch_rm.z, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h, use_raster = FALSE, column_title = "After batch correction")
dev.off()

##### Return matrix with row/column sorting as in heatmap
##### Cluster samples
data.h_batch_rm.z.heatmap <- data.h_batch_rm.z[, hc$labels[hc$order]]

##### Cluster genes
data.h_batch_rm.z.heatmap <- data.h_batch_rm.z.heatmap[hr$labels[hr$order], ]

##### Write data from the heatmap into a file
write.table(data.h_batch_rm.z.heatmap, paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_batch_rm_data.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
...
##### Cluster genes
hc <- hclust(as.dist(1-cor(data_before_batch_rm_heatmap, method="pearson")), method="ward.D2")

##### Cluster samples
hr <- hclust(as.dist(dist(data_before_batch_rm_heatmap, method="euclidean")), method="ward.D2")

##### Scale rows the data first (row scale)
data.h.z <- t(scale(t(data_before_batch_rm_heatmap)))
...
#### Heatmap (BEFORE batch correction)
Heatmap(data.h.z, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h, column_title = "Before batch correction")
```
...
p <- heatmaply(data.frame(data.h_batch_rm.z), Rowv = hr, Colv = rev(hc), k_col=1, k_row=1, colors = colorRampPalette(c("darkblue","darkblue","darkslateblue","darkslateblue","white","firebrick3","firebrick3","firebrick4","firebrick4"))(100), col_side_colors = col_side_colors, col_side_palette = col_side_palette, scale="row", trace="none", hide_colorbar = TRUE, fontsize_row = 8, fontsize_col = 8) %>%
layout(autosize = TRUE, width = 800, margin = list(l=150, r=50, b=150, t=50, pad=4), showlegend = FALSE)
...
for (i in 1:combNo) {
  contrasts <- c(contrasts, paste(paste(comb[1,i], comb[2,i], sep="_vs_"),paste(comb[1,i], comb[2,i], sep=" -"), sep="="))
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  plot(topGenes[,"logFC"],-log2(topGenes[,"adj.P.Val"]),pch=16,cex=0.5,xlab="Log2 fold-change",ylab="-log2(p-value)",main="",col="grey")
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  if ( length(topGenes_list[[i]] ) != 0 ) {
    ...
    topGenes_list_table[[i]] <- paste("\n**No differentially expressed genes** were detected in ", contrastNames[i], " comparison!\n", sep="")
  }
}
...
```{r DE_heatmap, warning = FALSE, echo = FALSE, comment = FALSE, message = FALSE, results = 'hide', fig.width = 12, fig.height = 10}
...
if ( length(topGenes.index) > 1 ) {
  ...
  pdf(paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_DE.pdf")), width = 12, height = 16)
  print(Heatmap(data.h.z, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h, use_raster = FALSE))
  dev.off()
  ...
  ##### Write data from the heatmap into a file
  write.table(data.h.z.heatmap, paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_DE_data.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
}
```

`r if ( length(topGenes.index) < 2 ) { c("\n**No differentially expressed genes** detected.\n") }`
...
if ( length(topGenes_list_upset) > 1 ) {
  print(upset(fromList(topGenes_list_upset), order.by = "freq", nsets = length(topGenes_list_upset), nintersects = NA, number.angles = 30, point.size = 10, line.size = 2, mainbar.y.label = "Comparisons intersections", sets.x.label = "DE genes", text.scale = 2))
...
`r if ( params$run_kegg ) { c("### KEGG pathways {.tabset}") }`

`r if ( params$run_kegg ) { c("Gene set enrichment analysis based on the *[KEGG](https://www.genome.jp/kegg/){target=\"_blank\"}* repository.") }`

```{r KEGG_gsea_analysis, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 3, results = 'hide', eval = params$run_kegg}
...
topGenes_list.pathway.genes <- list()
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  topGenes_list.pathway.genes[[i]] <- NA
  ...
  ##### Run analysis only for comparisons with significant results
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      if ( !is.null(gseKEGG.results) && nrow(gseKEGG.results) > 0 ) {
        ...
        ##### Collect genes for each pathway for GSVA
        for ( j in 1:nrow(top_gseKEGG.genes[[i]]) ) {
          
          ##### Skip already existing pathways
          if ( top_gseKEGG.genes[[i]]$Description[j] %!in% names(topGenes_list.pathway.genes) ) {
            topGenes_list.pathway.genes.list <- list(top_gseKEGG.genes[[i]]$core_enrichment[j])
            topGenes_list.pathway.genes.list[[1]] <- unique(unlist(strsplit(topGenes_list.pathway.genes.list[[1]], split='/', fixed=TRUE)))
            names(topGenes_list.pathway.genes.list)[1] <- top_gseKEGG.genes[[i]]$Description[j]
            
            topGenes_list.pathway.genes[[i]] <- c(topGenes_list.pathway.genes[[i]], topGenes_list.pathway.genes.list)
          }
        }
...
```{r KEGG_gsea_analysis_heatmap, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 3, results = 'hide', eval = params$run_kegg}
##### Create heatmap with GSEA enrichment scores calculated for each gene-set for individual samples
ht <- list()

for (i in 1:ncol(eb$p.value) ) {
  
    ##### Remove empty object in a list
    topGenes_list.pathway.genes[[i]] <- na.omit.list(topGenes_list.pathway.genes[[i]])
  
  if ( length(topGenes_list.pathway.genes[[i]]) > 1 ) {
  
    #gsva.results <- gsva(data, topGenes_list.pathway.genes[[i]], mx.diff=FALSE)  
    gsva.results <- gsva(data, topGenes_list.pathway.genes[[i]], method = "ssgsea")
    
    if ( nrow(gsva.results) > 1 ) {
      ##### Cluster genes
      hc <- hclust(as.dist(1-cor(gsva.results, method="pearson")), method="ward.D2")
      
      ##### Cluster samples
      hr <- hclust(as.dist(dist(gsva.results, method="euclidean")), method="ward.D2")
      
      ##### Generate heatmap (complexHeatmap)
      ##### Scale rows the data first (row scale)
      gsva.results.z <- t(scale(t(gsva.results)))
        
      ##### Define colour space ( https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html#colors )
      col.h = colorRamp2(seq(min(gsva.results.z), max(gsva.results.z), length = 3), c("blue", "#EEEEEE", "red"))
        
      ht[[i]] <- Heatmap(gsva.results.z, name = "Score", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h)
      
      pdf(paste0(KEGG_dir, "/gseKEGG_summary_heatmap_", colnames(eb$p.value)[i], ".pdf"), width = 12, height = 16)
      draw(ht[[i]], heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm"))
      dev.off()
      
      ##### Return matrix with row/column sorting as in heatmap
      ##### Cluster samples
      gsva.results.z.heatmap <- gsva.results.z[, hc$labels[hc$order]]
      
      ##### Cluster genes
      gsva.results.z.heatmap <- gsva.results.z.heatmap[hr$labels[hr$order], ]
    
      write.table(gsva.results.z.heatmap, paste0(KEGG_dir, "/gseKEGG_summary_heatmap_", colnames(eb$p.value)[i], ".txt"), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
...
`r if ( params$run_kegg && length(top_gseKEGG.upset) > 1 ) { c("Upset plot showing intersections of KEGG pathways enriched across multiple comparisons.") }`
...
if ( length(top_gseKEGG.upset) > 1 ) {
  print(upset(fromList(top_gseKEGG.upset), order.by = "freq", nsets = length(top_gseKEGG.upset), nintersects = NA, number.angles = 30, point.size = 10, line.size = 2, mainbar.y.label = "Comparisons intersections", sets.x.label = "Enriched pathways", text.scale = 2))
...
`r if ( params$run_kegg && length(contrastNames) > 1 ) { c("***") }`

```{r KEGG_gsea_visualisation, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 12, results="asis", eval = params$run_kegg}
setwd(KEGG_dir)
  ...
  ##### Run analysis only for comparisons with significant results
    ...
    if ( !is.na(top_gseKEGG[[i]]) ) {
      ...
      if ( !is.null(ht[[i]]) ) {
        ...
        ##### Summary heatmap
        cat(paste("\n\nHeatmap presenting Gene Set Variation Analysis (GSVA) **enrichment scores** in enriched pathways (*y-axis*) across all samples (*x-axis*) in **", contrastNames[i], "** comparison.\n\n", sep=""))
        print(draw(ht[[i]], heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm")))
      ...
      ##### Bar and dot plots
      cat(paste("\n\nDot plot (A) and heatmap (B) of enriched KEGG pathways in **", contrastNames[i], "** comparison. The gradient *color* scale and *point size* illustrate adjusted *p-values* and *number of differentially expressed genes* involved in individual pathways, respectively.\n\n", sep=""))
      ...
      for ( j in 1:nrow(top_gseKEGG.df[[i]]) ) {
        ...
        print(gseaplot2(top_gseKEGG[[i]], geneSetID = j, title = top_gseKEGG[[i]]$Description[j]))
        invisible(dev.off())
      ...
      cat(paste("\n**No enriched KEGG pathways** were detected in ", contrastNames[i], " comparison\n", sep=""))
      cat("\n***\n")
    }
  #} else {
  #  cat(paste("\n**No differentially expressed genes** were detected in ", contrastNames[i], " comparison\n", sep=""))
...
`r if ( params$run_DisGeNET ) { c("### DisGeNET pathways {.tabset}") }`

`r if ( params$run_DisGeNET ) { c("Gene set enrichment analysis based on the *[DisGeNET](https://www.disgenet.org/){target=\"_blank\"}* repository.") }`

```{r DisGeNET_gsea_analysis, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 4, results = 'hide', eval = params$run_DisGeNET}
##### Create folder for storing pathway analysis results plots
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      if ( !is.null(gseNCG.results) && nrow(gseNCG.results) > 0 ) {
        ...
        for ( j in 1:nrow(top_gseNCG.genes[[i]]) ) {
          ...
          ##### Skip already existing pathways
          if ( top_gseNCG.genes[[i]]$Description[j] %!in% names(topGenes_list.pathway.genes) ) {
            topGenes_list.pathway.genes.list <- list(top_gseNCG.genes[[i]]$core_enrichment[j])
            topGenes_list.pathway.genes.list[[1]] <- unique(unlist(strsplit(topGenes_list.pathway.genes.list[[1]], split='/', fixed=TRUE)))
            names(topGenes_list.pathway.genes.list)[1] <- top_gseNCG.genes[[i]]$Description[j]
...
```{r DisGeNET_gsea_analysis_heatmap, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 3, results = 'hide', eval = FALSE}
##### Create heatmap with GSEA enrichment scores calculated for each gene-set for individual samples
ht <- list()
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  if ( length(topGenes_list.pathway.genes[[i]]) > 1 ) {
    ...
    if ( nrow(gsva.results) > 1 ) {
      ...
      pdf(paste0(DisGeNET_dir, "/gseNCG_summary_heatmap_", colnames(eb$p.value)[i], ".pdf"), width = 12, height = 16)
      draw(ht[[i]], heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm"))
      dev.off()
      ...
      write.table(gsva.results.z.heatmap, paste0(DisGeNET_dir, "/gseNCG_summary_heatmap_", colnames(eb$p.value)[i], ".txt"), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
...
`r if ( params$run_DisGeNET ) { c("Upset plot showing intersections of DisGeNET pathways enriched across multiple comparisons.") }`

```{r DisGeNET_gsea_upset_plot, warning = FALSE, echo = TRUE, message = FALSE, results = 'hide', fig.width = 12, fig.height = 10, eval = params$run_DisGeNET}
if ( length(top_gseNCG.upset) > 1 ) {
  print(upset(fromList(top_gseNCG.upset), order.by = "freq", nsets = length(top_gseNCG.upset), nintersects = NA, number.angles = 30, point.size = 10, line.size = 2, mainbar.y.label = "Comparisons intersections", sets.x.label = "Enriched pathways", text.scale = 2))
...
`r if ( length(contrastNames) > 1 && params$run_DisGeNET ) { c("***") }`

```{r DisGeNET_gsea_visualisation, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 3, results="asis", eval = params$run_DisGeNET}
##### Now once the plots are ready show them in separate tabs
data.pathways <- list()
...
for( i in 1:ncol(eb$p.value) ){
  ...
  ##### Run analysis only for comparisons with significant results
    ...
    if ( !is.na(top_gseNCG[[i]]) ) {
      ...
      ##### Bar and dot plots
      cat(paste("\n\nDot plot (A) and heatmap (B) of enriched DisGeNET pathways in **", contrastNames[i], "** comparison. The gradient *color* scale and *point size* illustrate adjusted *p-values* and *number of differentially expressed genes* involved in individual pathways, respectively.\n\n", sep=""))
      ...
      for ( j in 1:nrow(top_gseNCG.df[[i]]) ) {
        ...
        print(gseaplot2(top_gseNCG[[i]], geneSetID = j, title = top_gseNCG[[i]]$Description[j]))
        invisible(dev.off())
      ...
      cat(paste("\n**No enriched DisGeNET pathways** were detected in ", contrastNames[i], " comparison\n", sep=""))
      cat("\n***\n")
    }
  #} else {
  #  cat(paste("\n**No differentially expressed genes** were detected in ", contrastNames[i], " comparison\n", sep=""))
...
`r if ( params$run_kegg ) { c("Over-representation analysis based on the *[KEGG](https://www.genome.jp/kegg/){target=\"_blank\"}* repository.") }`

```{r KEGG_over_representation_analysis, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 4, results = 'hide', eval = params$run_kegg}
...
`r if ( params$run_kegg && length(top_enrichKEGG.upset) > 1 ) { c("Upset plot showing intersections of KEGG pathways over-representated across multiple comparisons.") }`

```{r KEGG_over_representation_upset_plot, warning = FALSE, echo = TRUE, message = FALSE, results = 'hide', fig.width = 12, fig.height = 10, eval = params$run_kegg}
if ( length(top_enrichKEGG.upset) > 1 ) {
  print(upset(fromList(top_enrichKEGG.upset), order.by = "freq", nsets = length(top_enrichKEGG.upset), nintersects = NA, number.angles = 30, point.size = 10, line.size = 2, mainbar.y.label = "Comparisons intersections", sets.x.label = "Over-representated pathways", text.scale = 1.8))
...
```{r KEGG_over_representation_visualisation, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 3, results="asis", eval = params$run_kegg}
setwd(KEGG_dir)
...
for( i in 1:ncol(eb$p.value) ){
  ...
  if ( length(topGenes_list[[i]]) > 1 ) {
    ...
    if ( !is.na(top_enrichKEGG[[i]]) ) {
      ...
      cat(paste("\n**No over-represented KEGG pathways** were detected in ", contrastNames[i], " comparison\n", sep=""))
      cat("\n***\n")
    }
  } else {
    cat(paste("\n**No differentially expressed genes** were detected in ", contrastNames[i], " comparison\n", sep=""))
...
`r if ( params$run_DisGeNET ) { c("Over-representation analysis based on the *[DisGeNET](https://www.disgenet.org/){target=\"_blank\"}* repository.") }`

```{r DisGeNET_over_representation_analysis, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 3, results = 'hide', eval = params$run_DisGeNET}
##### Create lists to store pathways enrichment results for each comparison
enrichDGN <- list()
enrichDGN.df <- list()
...
`r if ( params$run_DisGeNET ) { c("Upset plot showing intersections of DisGeNET pathways over-representated across multiple comparisons.") }`

```{r DisGeNET_over_representation_upset_plot, warning = FALSE, echo = TRUE, message = FALSE, results = 'hide', fig.width = 12, fig.height = 10, eval = params$run_DisGeNET}
if ( length(enrichDGN.upset) > 1 ) {
  print(upset(fromList(enrichDGN.upset), order.by = "freq", nsets = length(enrichDGN.upset), nintersects = NA, number.angles = 30, point.size = 10, line.size = 2, mainbar.y.label = "Comparisons intersections", sets.x.label = "Over-representated pathways", text.scale = 1.8))
...
```{r DisGeNET_over_representation__visualisation, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 3, results="asis", eval = params$run_DisGeNET}
##### Now once the plots are ready show them in separate tabs
data.pathways <- list()
...
for( i in 1:ncol(eb$p.value) ){
  ...
  if ( length(topGenes_list[[i]]) > 1 ) {
    ...
    if ( !is.na(enrichDGN[[i]]) ) {
      ...
      cat(paste("\n**No over-represented DisGeNET pathways** were detected in ", contrastNames[i], " comparison\n", sep=""))
      cat("\n***\n")
    }
  } else {
    cat(paste("\n**No differentially expressed genes** were detected in ", contrastNames[i], " comparison\n", sep=""))
...
`r if ( params$run_reactome ) { c("## MSigDB and Reactome pathways {.tabset}") } else { c("## MSigDB pathways {.tabset}") }`

### MSigDB (H) {.tabset}

#### Heatmap

Heatmap presenting enrichment scores for [Molecular Signatures Database (MSigDB)](https://www.gsea-msigdb.org/gsea/msigdb){target="_blank"} **hallmark gene sets** (**H**) (*y-axis*) across all samples (*x-axis*).

```{r MSigDB_H_pathways_heatmap, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 12, results = 'hide'}
##### Create folder for storing pathway analysis results plots
MSigDB_dir <- paste(params$output_dir, "MSigDB_pathways", sep = "/")
if ( !file.exists(MSigDB_dir) ) {
  dir.create(MSigDB_dir, recursive=TRUE)
}
setwd(MSigDB_dir)

ht <- NULL

##### Retrieve a data frame of gene sets from MSigDB and their member genes
gene_sets = msigdbr(species = "Homo sapiens", category = "H")

msigdbr_list_h = split(x = gene_sets$gene_symbol, f = gene_sets$gs_name)

##### Create heatmap with GSEA enrichment scores calculated for each gene-set for individual samples
#gsva.results <- gsva(data, msigdbr_list_h, mx.diff=FALSE)
gsva.results <- gsva(data, msigdbr_list_h, method = "ssgsea")
...
if ( nrow(gsva.results) > 1 ) {
  ...
  ht <- Heatmap(gsva.results.z, name = "Score", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h)
  
  draw(ht, heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm"))
    
  pdf(paste0(MSigDB_dir, "/MSigDB_H_summary_heatmap.pdf"), width = 12, height = 12)
  draw(ht, heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm"))
  dev.off()
  ...
  write.table(gsva.results.z.heatmap, paste0(MSigDB_dir, "/MSigDB_H_summary_heatmap.txt"), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
  
} else {
  cat(paste("\n**No MSigDB pathways** were detected\n", sep=""))
}
```
...
Table presenting enrichment scores for [Molecular Signatures Database (MSigDB)](https://www.gsea-msigdb.org/gsea/msigdb){target="_blank"} **hallmark gene sets** (**H**) (*y-axis*) across all samples (*x-axis*).

```{r MSigDB_H_pathways_table, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, results="asis"}
##### Trim pathways names length to 50 characters
rownames(gsva.results) <- substring(rownames(gsva.results), 1, 50)

DT::datatable( data = gsva.results, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;'), filter = "top", extensions = c('Buttons','FixedColumns','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, fixedColumns = list(leftColumns = 1), deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE ) %>%
      DT::formatStyle( columns = colnames(gsva.results), 'text-align' = 'center' ) %>%
      DT::formatRound(columns = colnames(gsva.results), 2)
```
...
Heatmap presenting enrichment scores for [Molecular Signatures Database (MSigDB)](https://www.gsea-msigdb.org/gsea/msigdb){target="_blank"} **oncogenic signature gene sets** (**C6**) (*y-axis*) across all samples (*x-axis*).

```{r MSigDB_C6_pathways_heatmap, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 12, results = 'hide'}
##### Retrieve a data frame of gene sets from MSigDB and their member genes
gene_sets = msigdbr(species = "Homo sapiens", category = "C6")

msigdbr_list_c6 = split(x = gene_sets$gene_symbol, f = gene_sets$gs_name)

##### Create heatmap with GSEA enrichment scores calculated for each gene-set for individual samples
#gsva.results <- gsva(data, msigdbr_list_c6, mx.diff=FALSE)
gsva.results <- gsva(data, msigdbr_list_c6, method = "ssgsea")
...
if ( nrow(gsva.results) > 1 ) {
  ...
  pdf(paste0(MSigDB_dir, "/MSigDB_C6_summary_heatmap.pdf"), width = 12, height = 30)
  draw(ht, heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm"))
  dev.off()
  ...
  write.table(gsva.results.z.heatmap, paste0(MSigDB_dir, "/MSigDB_C6_summary_heatmap.txt"), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
...
Table presenting enrichment scores for [Molecular Signatures Database (MSigDB)](https://www.gsea-msigdb.org/gsea/msigdb){target="_blank"} **oncogenic signature gene sets** (**C6**) (*y-axis*) across all samples (*x-axis*).

```{r MSigDB_C6_pathways_table, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, results="asis"}
##### Trim pathways names length to 50 characters
rownames(gsva.results) <- substring(rownames(gsva.results), 1, 50)
...
`r if ( params$run_reactome ) { c("***") }`

`r if ( params$run_reactome ) { c("### Reactome {.tabset}") }`

`r if ( params$run_reactome ) { c("#### Heatmap") }`

`r if ( params$run_reactome ) { c("Heatmap presenting enrichment scores for  *[Reactome](https://reactome.org/PathwayBrowser){target=\"_blank\"}* pathways (*y-axis*) across all samples (*x-axis*).") }`

```{r Reactome_pathways_heatmap, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 12, results = 'hide', eval = params$run_reactome}
##### Create folder for storing pathway analysis results plots
Reactome_dir <- paste(params$output_dir, "Reactome_pathways", sep = "/")
if ( !file.exists(Reactome_dir) ) {
  dir.create(Reactome_dir, recursive=TRUE)
}
setwd(Reactome_dir)

#####  Map Reactome pathway identifiers to Entrez Gene identifiers
reactome_pathways <- as.list(reactomePATHID2EXTID)

##### Map Reactome pathway identifiers to Reactome pathway names
reactome_pathways_names <- as.list(reactomePATHID2NAME)

##### Keep only Homo sapiens pathways
reactome_pathways <- reactome_pathways[ grep("R-HSA-", names(reactome_pathways), value = TRUE)]
reactome_pathways_names <- reactome_pathways_names[ grep("R-HSA-", names(reactome_pathways_names), value = TRUE)]

##### Make sure that pathways IDs match pathways names
reactome_pathways <- reactome_pathways[ names(reactome_pathways) %in% names(reactome_pathways_names) ]
reactome_pathways_names <- reactome_pathways_names[ names(reactome_pathways_names) %in% names(reactome_pathways) ]

##### Remove "Homo sapiens" from the pathway name
reactome_pathways_names  <- lapply(reactome_pathways_names,function(x) gsub("Homo sapiens: ", "", x)) 


##### Change the Reactome pathway identifiers to Reactome pathway names
reactome_pathways_names <- reactome_pathways_names[ names(reactome_pathways) ]
names(reactome_pathways) <- reactome_pathways_names
...
gene_info <- AnnotationDbi::select(org.Hs.eg.db, keys = keys, columns = c("SYMBOL"), keytype = "ENTREZID")

##### Substitute ENTREZIDs with Gene symbols 
reactome_pathways.gene_symbols <- lapply(reactome_pathways, function(x) gene_info$SYMBOL[match(x, gene_info$ENTREZID)] )

##### Remove unmapped genes (NAs)
reactome_pathways.gene_symbols <- lapply(reactome_pathways.gene_symbols, function(x) x[!is.na(x)] )

##### Create heatmap with GSEA enrichment scores calculated for each gene-set for individual samples
#gsva.results <- gsva(data, reactome_pathways.gene_symbols, mx.diff=FALSE)
gsva.results <- gsva(data, reactome_pathways.gene_symbols, method = "ssgsea")
...
if ( nrow(gsva.results) > 1 ) {
  ...
  ht <- Heatmap(gsva.results.z, name = "Score", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h, show_row_names = FALSE)
  
  draw(ht, heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm"))
  
  pdf(paste0(Reactome_dir, "/Reactome_summary_heatmap.pdf"), width = 12, height = 12)
  draw(ht, heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm"))
  dev.off()
  ...
  write.table(gsva.results.z.heatmap, paste0(Reactome_dir, "/Reactome_summary_heatmap.txt"), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
    
} else {
  cat(paste("\n**No Reactome pathways** were detected\n", sep=""))
}
```

```{r pathways_genes, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 12, results = 'hide'}
if ( params$run_reactome ) { 
  pathways.genes <- c(msigdbr_list_h, msigdbr_list_c6 ,reactome_pathways.gene_symbols)
} else {
  pathways.genes <- c(msigdbr_list_h, msigdbr_list_c6)
}
pathways.genes <- pathways.genes[ unique(names(pathways.genes)) ]
```

`r if ( params$run_reactome ) { c("***") }`

`r if ( params$run_reactome ) { c("#### Table") }`

`r if ( params$run_reactome ) { c("Table presenting enrichment scores for  *[Reactome](https://reactome.org/PathwayBrowser){target=\"_blank\"}* pathways (*y-axis*) across all samples (*x-axis*).") }`

```{r Reactome_pathways_table, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, results="asis", eval = params$run_reactome}
##### Trim pathways names length to 50 characters
rownames(gsva.results) <- substring(rownames(gsva.results), 1, 50)
...
`r if ( runPOI ) { c("### Selected pathways") }`

`r if ( runPOI ) { c("Heatmap presenting enrichment scores for **pathways of interest** (*y-axis*) across all samples (*x-axis*).") }`

`r if ( runPOI ) { c("***") }`

`r if ( runPOI ) { paste0("<details>") }`
`r if ( runPOI ) { paste0("<summary>Selected pathways</summary>") }`
`r if ( runPOI ) { paste0("<font size=\"2\">") }`

`r if ( runPOI ) { paste(poi_list, collapse = ", ") }`

`r if ( runPOI ) { paste0("</font>") }`
`r if ( runPOI ) { paste0("</details>") }`

`r if ( runPOI ) { c("***") }`

```{r Selected_pathways_heatmap, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 12, results = 'hide', eval = runPOI}
##### Create folder for storing pathway analysis results plots
Selected_pathways_dir <- paste(params$output_dir, "Selected_pathways", sep = "/")
if ( !file.exists(Selected_pathways_dir) ) {
  dir.create(Selected_pathways_dir, recursive=TRUE)
}
setwd(Selected_pathways_dir)

##### Keep only pathways of interest
pathways.selected <- pathways.genes[ match(poi_list, names(pathways.genes)) ]

##### Create heatmap with GSEA enrichment scores calculated for each gene-set for individual samples
#gsva.results <- gsva(data, pathways.selected, mx.diff=FALSE)
gsva.results <- gsva(data, pathways.selected, method = "ssgsea")

##### Set heatmaps height
if ( length(pathways.selected) < 10 ) {
  heatmap_poi_height <- 4
} else {
  heatmap_poi_height <- 8
}
...
if ( nrow(gsva.results) > 1 ) {
  ...
  pdf(paste0(Selected_pathways_dir, "/Selected_pathways_summary_heatmap.pdf"), width = 12, height = heatmap_poi_height)
  draw(ht, heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm"))
  dev.off()
  ...
  write.table(gsva.results.z.heatmap, paste0(Selected_pathways_dir, "/Selected_pathways_summary_heatmap.txt"), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
    
} else {
  cat(paste("\n**None of the selected pathways** were detected\n", sep=""))
}
```
...
* Enrichment score: **`r  params$eScoreThreshold`**
* p-value: **`r  params$pThreshold`**

`r if ( params$adjMethod_pathways=="none" ) { c("**NOTE**, p-values were not corrected for multiple testing!") } else { c(paste0("P-values were corrected multiple testing using **", params$adjMethod_pathways, "** method.")) }`

</font> 
</details>

```{r DE_model_fit_pathways, comment = NA, message=FALSE, warning=FALSE, results = 'hide'}
##### Create matrix with GSEA enrichment scores calculated for each gene-set for individual samples
#gsva.results <- gsva(data, pathways.selected, mx.diff=FALSE)
gsva.results <- gsva(data, pathways.genes, method = "ssgsea")

##### For better presentation keep only the first 6 words of each pathway name
gsva.results.names <- word(rownames(gsva.results),start=1,end=6,sep=fixed(" "))
gsva.results.short_names <- is.na(word(rownames(gsva.results),start=1,end=6,sep=fixed(" ")))
gsva.results.names[ gsva.results.short_names ] <- rownames(gsva.results)[ gsva.results.short_names ]
rownames(gsva.results) <- gsva.results.names

##### Keep only unique pathways
gsva.results <- gsva.results[ unique(rownames(gsva.results)), ]


#####  Fit linear model to combined data, given the above design
if ( all(is.na(targetFile$Replicates)) || all(targetFile$Replicates == "-") ) {
  fit <- lmFit(gsva.results, mod)
} else {
  cat("Dealing with technical replicates...\n")
  ...
  corfit <- duplicateCorrelation(gsva.results, design=mod, ndups = 1, block = biolrep)
  fit <- lmFit(gsva.results, mod, block = biolrep, correlation = corfit$consensus.correlation)
}
...
```{r DE_results_pathways, comment = NA, message=FALSE, warning=FALSE, results = 'hide'}
##### Differenatial expression analysis results visualisation
for (i in 1:ncol(eb$p.value) ) {
    
  #####  P-value histograms
  png(file = paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_pathways_P_hist.png", sep=""), width = 6, height = 4, units = "in", res = 300, pointsize = 10)
  histogram <- hist(eb$p.value[,i], breaks=seq(0,1,by= 0.01), main="", xlab="p-value")
  exprected_p.value <- mean(histogram$counts)+(sd(histogram$counts)*1)
  abline(v=params$pThreshold_pathways,col="red")
  abline(h=exprected_p.value,col="blue")
  dev.off()
  
  
  #####  Volcano plots of Enrichment scores versus significance (adjusted p-values, +label top 10)
  topGenes <- topTable(eb, coef=colnames(eb)[i], adjust=params$adjMethod_pathways, sort.by="none", number=nrow(gsva.results))
  names(topGenes) <- gsub("logFC", "score", names(topGenes))
  
  png(file = paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_pathways_volcano_plot", ".png", sep = ""), width = 6, height = 4, units = "in", res = 300, pointsize = 10)
  plot(topGenes[,"score"],-log2(topGenes[,"adj.P.Val"]),pch=16,cex=0.5,xlab="Enrichment score difference",ylab="-log2(p-value)",main="",col="grey", xlim=c(min(topGenes[,"score"])-0.5, max(topGenes[,"score"])+0.5))
  #####  Highlight genes with Enrichment scores above specified threshold
  points(topGenes[abs(topGenes[,"score"])>params$eScoreThreshold,"score"],-log2(topGenes[abs(topGenes[,"score"])>params$eScoreThreshold,"adj.P.Val"]),cex=0.5,pch=16)
  abline(h=-log2(params$pThreshold_pathways),col="red", lty = 2)
  #####  Label top 10 most significant genes
  ord <- order(-log2(topGenes[,"adj.P.Val"]),decreasing=TRUE)
  top10 <- ord[1:10]
  text(topGenes[top10,"score"],-log2(topGenes[top10,"adj.P.Val"]), labels=rownames(gsva.results[top10,]),cex=0.6,col="blue")
  dev.off()
}
```

```{r DE_overlap_pathways, comment = NA, message=FALSE, warning=FALSE, results = 'asis'}
eb.decideTests <- decideTests(eb, method="separate", adjust.method=params$adjMethod_pathways, p.value=params$pThreshold_pathways, lfc=params$eScoreThreshold)

eb.decideTests <- as.data.frame(eb.decideTests)

##### Record Enrichment scores and p-values for all pathways across all comparisons
topGenes_list_log2FC <- NULL
topGenes_list_p <- NULL
...
write.table(eb.decideTests, file=paste(paste0(params$output_dir, "/", paste0(params$results_name)),"_pathways_decideTests.txt", sep = ""), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)

##### Create list to store DE results for ALL pathways for each comparison
topGenes_list_all <- list()

#####  Write the analysis results into a file
for (i in 1:ncol(eb$p.value) ) {
    
	topGenes <- topTable(eb, coef=colnames(eb)[i], adjust=params$adjMethod_pathways,  sort.by="p", number=nrow(gsva.results))
	names(topGenes) <- gsub("logFC", "score", names(topGenes))
  topGenes <- topGenes[,colnames(topGenes) %in% c("score","t","B","P.Value","adj.P.Val")]
  topGenes <- topGenes[ ,c("score","t","B","P.Value","adj.P.Val")]
  colnames(topGenes) <- c("score", "t-statistic", "B-statistic", "p-value", paste("p-value", params$adjMethod_pathways,sep="_"))
  topGenes_list_all[[i]] <- topGenes
    
  write.table(topGenes, file=paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_pathways_topTable.txt", sep=""),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
    
  ##### Save Enrichment score values for DE pathways
  topGenes <- topTable(eb, coef=colnames(eb)[i], adjust=params$adjMethod_pathways,  sort.by="none", number=nrow(gsva.results))
  names(topGenes) <- gsub("logFC", "score", names(topGenes))
  topGenes <- topGenes[,colnames(topGenes) %in% c("score","t","B","P.Value","adj.P.Val")]
  topGenes <- topGenes[ ,c("score","t","B","P.Value","adj.P.Val")]
  ...
  topGenes_list_log2FC <- cbind(topGenes_list_log2FC, topGenes$score)
  
  ##### Save Enrichment score values for DE pathways
  topGenes_list_p <- cbind(topGenes_list_p, topGenes[ , paste("p-value", params$adjMethod_pathways,sep="_") ])
}

##### Write Enrichment score and p-values for all pathways across all comparisons into a file
rownames(topGenes_list_log2FC) <- rownames(topGenes)
colnames(topGenes_list_log2FC) <- colnames(eb)
write.table(topGenes_list_log2FC , file=paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_DE_pathways_enrichment_score.txt", sep=""),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)

rownames(topGenes_list_p) <- rownames(topGenes)
colnames(topGenes_list_p) <- colnames(eb)
write.table(topGenes_list_p , file=paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_DE_pathways_p_value.txt", sep=""),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  topGenes_list[[i]] <- topTable(eb, coef=colnames(eb)[i], adjust=params$adjMethod_pathways,  sort.by="p", p.value=params$pThreshold_pathways, lfc=params$eScoreThreshold, number=nrow(gsva.results))
  names(topGenes_list[[i]]) <- gsub("logFC", "score", names(topGenes_list[[i]]))
  	
  if ( length(topGenes_list[[i]] ) != 0 ) {
    
    topGenes_list[[i]]  <- topGenes_list[[i]] [,colnames(topGenes_list[[i]] ) %in% c("score","t","B","P.Value","adj.P.Val")]
    topGenes_list[[i]]  <- topGenes_list[[i]] [ ,c("score","t","B","P.Value","adj.P.Val")]
    colnames(topGenes_list[[i]] ) <- c("score", "t-statistic", "B-statistic", "p-value", paste("p-value", params$adjMethod_pathways,sep="_"))
    
    write.table(topGenes_list[[i]] , file=paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_DE_pathways.txt", sep=""),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
    
    ##### Save the normalised expression data for DE pathways
    data2plot_topGenes <- gsva.results[ rownames(topGenes_list[[i]]), ]
    
    write.table(data2plot_topGenes , file=paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_DE_pathways_exprs.txt", sep=""),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
    
    ##### Save Enrichment score and p-values for DE pathways
    write.table(topGenes_list_log2FC[ rownames(topGenes_list[[i]]), ] , file=paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_DE_pathways_enrichment_score.txt", sep=""),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
    
    write.table(topGenes_list_p[ rownames(topGenes_list[[i]]), ] , file=paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_DE_pathways_p_value.txt", sep=""),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
    
    ##### For Upset plot
    topGenes_list_upset[[i]] <- rownames(topGenes_list[[i]])
    
    ##### Generate dataTable
    topGenes4table <- topGenes_list[[i]][ , c("score", "t-statistic", "B-statistic", "p-value", paste("p-value", params$adjMethod_pathways,sep="_"))]
    topGenes_list_table[[i]] <- DT::datatable( data = topGenes4table, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;'), filter = "top", extensions = c('Buttons','FixedColumns','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, fixedColumns = list(leftColumns = 1), deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE ) %>%
        DT::formatStyle( columns = names(topGenes4table), 'text-align' = 'center' ) %>%
        DT::formatRound(columns = c("score", "t-statistic", "B-statistic"), 2)
...
Heatmap presenting expression levels of `r length(topGenes.index)` **differentially expressed pathways** (*y-axis*) across all samples (*x-axis*).

##### Static

```{r DE_heatmap_pathways, warning = FALSE, echo = FALSE, comment = FALSE, message = FALSE, results = 'hide', fig.width = 12, fig.height = 10}
##### Make sure that any DE genes were detected
if ( length(topGenes.index) > 1 ) {
  data.h <- data.frame(gsva.results[ rownames(gsva.results) %in% topGenes.index,])
  ...
  ht <- Heatmap(data.h.z, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h)
  
  draw(ht, heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm"))
  
  pdf(paste0(params$output_dir, "/", paste0(params$results_name, "_pathways_heatmap_DE.pdf")), width = 12, height = 8)
  draw(ht, heatmap_legend_side = "left", annotation_legend_side = "left", padding = unit(c(5, 5, 5, 60), "mm"))
  dev.off()
  ...
  ##### Write data from the heatmap into a file
  write.table(data.h.z.heatmap, paste0(params$output_dir, "/", paste0(params$results_name, "_pathways_heatmap_DE_data.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
}
```
...
```{r DE_heatmap_interactive_pathways, echo = TRUE, comment = TRUE, message = TRUE, warning = FALSE, results = 'asis', fig.width = 12, fig.height = 16}
suppressMessages(library(heatmaply))
...
if ( length(topGenes.index) > 1 ) {
  ...
  ##### Save the heatmap as html (PLOTLY)
  saveWidgetFix(output[[1]], file = paste0(params$output_dir, "/", paste0(params$results_name, "_pathways_heatmap_DE.html")), selfcontained = TRUE)
} else {
  output[[1]] <- c("\n**No differentially expressed pathways** detected\n")
}
...
```{r DE_venn_pathways, comment = NA, message=FALSE, warning=FALSE, results = 'hide', eval=FALSE}
...
```{r DE_upset_plot_pathways, warning = FALSE, echo = TRUE, message = FALSE, results = 'hide', fig.width = 12, fig.height = 10}
if ( length(topGenes_list_upset) > 1 ) {
  print(upset(fromList(topGenes_list_upset), order.by = "freq", nsets = length(topGenes_list_upset), nintersects = NA, number.angles = 30, point.size = 10, line.size = 2, mainbar.y.label = "Comparisons intersections", sets.x.label = "DE pathways", text.scale = 2))
  
  png(file = paste0(params$output_dir, "/", paste0(params$results_name, "_pathways_upset_DE.png"), sep = ""), width = 12, height = 10, units = "in", res = 300, pointsize = 10)
  print(upset(fromList(topGenes_list_upset), order.by = "freq", nsets = length(topGenes_list_upset), nintersects = NA, number.angles = 30, point.size = 10, line.size = 2, mainbar.y.label = "Comparisons intersections", sets.x.label = "DE pathways", text.scale = 2))
  dev.off()
}
```
...
```{r DE_summary_table_pathways, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, results="asis"}
DT::datatable( data = eb.decideTests, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;'), filter = "top", extensions = c('Buttons','FixedColumns','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, fixedColumns = list(leftColumns = 1), deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE ) %>%
      DT::formatStyle( columns = names(eb.decideTests), 'text-align' = 'center' ) %>%
...
```{r DE_results_visualisation_pathways, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, results="asis"}
##### Now once the plots are ready show them in separate tabs
for( i in 1:ncol(eb$p.value) ){
  ...
  cat(paste0("![](", paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_pathways_volcano_plot.png)", sep="")), "\n")
  cat("\n***\n")
  ...
  cat(paste0("![](", paste(paste0(params$output_dir, "/", paste0(params$results_name)), "_", colnames(eb$p.value)[i], "_pathways_P_hist.png)", sep="")), "\n")
  cat("\n***\n")
}
```
