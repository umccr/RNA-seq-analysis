...
# WORK IN PROGRESS
# Also the counts data should be corrected for batch effects as this data is to be used for DE analysis

# IN edge R that kind of formula should be used  (see page 41 in https://www.bioconductor.org/packages/release/bioc/vignettes/edgeR/inst/doc/edgeRUsersGuide.pdf )
# design <- model.matrix(~Batch+Treatment) 


# Apply both LIMMA (lmFit and contrasts.fit; https://www.bioconductor.org/packages/devel/workflows/vignettes/RNAseq123/inst/doc/limmaWorkflow.html#differential-expression-analysis  ) and EDGER (glmQLFTest; https://www.bioconductor.org/packages/release/bioc/vignettes/edgeR/inst/doc/edgeRUsersGuide.pdf ) DE models
# and then show overlap
...
if ( length(unique(batches)) > 1 ) {
  ...
  batch_rm_status <- params$batch_rm
} else {
...
write.table(datasets.list[[dataset]][["batch_effect_corrected"]], file = paste0(params$output_dir, "/", paste0(params$results_name, ".combined.txt")), sep="\t", quote=FALSE, row.names=FALSE, append = FALSE )

##### Write list genes into a file
write.table(rownames(datasets.list[[dataset]][["batch_effect_corrected"]]), file = paste0(params$output_dir, "/", params$results_name,".combined_genes.txt"), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
```

`r if ( params$batch_rm ) { c("***") }`

`r if ( params$batch_rm ) { c("## Batch-effects correction") }`
...
```{r pca_batch_effect, comment = NA, message=FALSE, warning=FALSE, eval=params$batch_rm}
suppressMessages(library(plotly))
...
datasets.list[[dataset]][["pca_combined_data_processed"]] <- pca(data=datasets.list[[dataset]][["combined_data_processed"]], targets=target)
datasets.list[[dataset]][["pca_batch_effect_corrected"]] <- pca(data=datasets.list[[dataset]][["batch_effect_corrected"]], targets=target)
...
if ( all(is.na(targetFile$Replicates)) || all(targetFile$Replicates == "-") ) {
  ...
  fit <- lmFit(DGEdata.voom, mod, block = biolrep, cor = corfit$consensus)
}
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  colnames(topGenes) <- c("log2FC", "t-statistic", "p-value", paste("p-value", params$adjMethod,sep="_"))
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  if ( length(topGenes_list[[i]] ) != 0 ) {
    ...
    topGenes_list[[i]]  <- topGenes_list[[i]] [,colnames(topGenes_list[[i]] ) %in% c("ENTREZID", "logFC","t","P.Value","adj.P.Val")]
    colnames(topGenes_list[[i]] ) <- c("ENTREZID", "log2FC", "t-statistic", "p-value", paste("p-value", params$adjMethod,sep="_"))
    ...
    ##### Generate dataTable
    topGenes4table <- topGenes_list[[i]][ , c("log2FC", "t-statistic", "p-value", paste("p-value", params$adjMethod,sep="_"))]
    topGenes_list_table[[i]] <- DT::datatable( data = topGenes4table, caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;'), filter = "top", extensions = c('Buttons','FixedColumns','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, fixedColumns = list(leftColumns = 1), deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE ) %>%
        ...
        DT::formatRound(columns = c("log2FC", "t-statistic"), 2)
...
#### FIX in cases where no DE genes were detecetd if ( !is.null(topGenes.index) ) {
...
```{r DE_heatmap_interactive, warning = FALSE, echo = TRUE, message = FALSE, results = 'as.is', fig.width = 12, fig.height = 8}
suppressMessages(library(heatmaply))
...
##### Save the heatmap as html (PLOTLY)
saveWidgetFix(p, file = paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_DE.html")), selfcontained = TRUE)
...
`r if ( length(contrastNames) > 1 ) { c("#### Upset plot") }`

`r if ( length(contrastNames) > 1 ) { c("Upset plot showing intersections of differentially expressed (DE) genes detected across multiple comparisons.") }`

```{r DE_upset_plot, warning = FALSE, echo = TRUE, message = FALSE, results = 'hide', fig.width = 12, fig.height = 10}
if ( length(contrastNames) > 1 ) {
...
`r if ( length(contrastNames) > 1 ) { c("#### Table") }`

```{r DE_summary_table, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, results="asis"}
##### Show DE results across mutliple comparisons
if ( length(contrastNames) > 1 && !is.null(topGenes.index) ) {
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      if ( !is.null(enrichKEGG.results) && nrow(enrichKEGG.results) > 0 ) {
        ...
        for ( j in 1:nrow(top_enrichKEGG.genes[[i]]) ) {
          ...
          write.table(topGenes_list_p[ rownames(topGenes_list_log2FC) %in% topGenes_list.pathway, ] , file=paste0(KEGG_dir, "/enrichKEGG_", colnames(eb$p.value)[i], "_", make.names(top_enrichKEGG.genes[[i]]$Description[j]), "_DE_genes_p_value.txt"),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
        }
...
`r if ( length(contrastNames) > 1 ) { c("Upset plot showing intersections of KEGG pathways over-representated across multiple comparisons.") }`
...
for( i in 1:ncol(eb$p.value) ){
  ...
  if ( length(topGenes_list[[i]]) > 1 ) {
    ...
    if ( !is.na(top_enrichKEGG[[i]]) ) {
      ...
      cat(paste("Visualization of KEGG pathways over-represented in **", contrastNames[i], "** comparison. The gradient *color* scale illustrates expression *log2 fold-change* values observed in that comparison.\n\n", sep=""))
      ...
      for ( j in 1:nrow(top_enrichKEGG.df[[i]]) ) {
        ...
        if ( rownames(top_enrichKEGG.df[[i]])[j] %!in% c("hsa04723", "hsa04215", "hsa05206") ) {
          ...
          cat(paste0("![](", paste(KEGG_dir, "/", rownames(top_enrichKEGG.df[[i]])[j], ".pathview.png)", sep="")), "\n")
        } else {
      ...
      for ( j in 1:nrow(top_enrichKEGG.df[[i]]) ) {
        ...
        png(file = paste(KEGG_dir, "/summary_", rownames(top_enrichKEGG.df[[i]])[j], ".network.png", sep=""))
        print(cnetplot(topPathways.genes, foldChange=data.pathways[[i]]))
        invisible(dev.off())
        
        cat(paste0("![](", paste(KEGG_dir, "/summary_", rownames(top_enrichKEGG.df[[i]])[j], ".network.png)", sep="")), "\n")
...
for (i in 1:ncol(eb$p.value) ) {
  ...
  if ( !is.na(topGenes_list[[i]]) ) {
    ...
    ##### Remove genes with invalid ENTREZIDs and check if at least two genes have valid ENTREZIDs
    genes.ENTREZID <- topGenes_list[[i]]$log2FC[ !is.na(topGenes_list[[i]]$ENTREZID) ]
    names(genes.ENTREZID) <- topGenes_list[[i]]$ENTREZID[ !is.na(topGenes_list[[i]]$ENTREZID) ]
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      if ( !is.null(gseKEGG.results) && nrow(gseKEGG.results) > 0 ) {
        ...
        for ( j in 1:nrow(top_gseKEGG.genes[[i]]) ) {
          ...
          write.table(topGenes_list_p[ rownames(topGenes_list_log2FC) %in% topGenes_list.pathway, ] , file=paste0(KEGG_dir, "/gseKEGG_", colnames(eb$p.value)[i], "_", make.names(top_gseKEGG.genes[[i]]$Description[j]), "_DE_genes_p_value.txt"),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
        }
...
`r if ( length(contrastNames) > 1 ) { c("Upset plot showing intersections of KEGG pathways enriched across multiple comparisons.") }`

```{r KEGG_gsea_analysis_upset_plot, warning = FALSE, echo = TRUE, message = FALSE, results = 'hide', fig.width = 12, fig.height = 10}
if ( length(contrastNames) > 1 ) {
  ...
  if ( length(topGenes_list[[i]]) > 1 ) {
    ...
    if ( !is.na(top_gseKEGG[[i]]) ) {
      ...
      for ( j in 1:nrow(top_gseKEGG.df[[i]]) ) {
        ...
        ##### NOTE: Some pathways are problematic. Skip these pathways
        # https://support.bioconductor.org/p/122172/
        if ( rownames(top_gseKEGG.df[[i]])[j] %!in% c("hsa04723", "hsa04215", "hsa05206") ) {
          ...
          cat(paste0("![](", paste(KEGG_dir, "/", rownames(top_gseKEGG.df[[i]])[j], ".pathview.png)", sep="")), "\n")
        } else {
      ...
      cat(paste("Plots presenting GSEA results. The *top* panel illustrates running *enrichment score* (ES), which reflects the degree to which a gene set is overrepresented at the top or bottom of a ranked list of genes, for the ", top_gseKEGG.df[[i]]$Description[j], " gene set as the analysis walks down the ranked list. Gene sets with a distinct peak at the beginning or end of the ranked list are generally the most interesting. The *bottom* panel shows the value of the ranking metric as you move down the list of ranked genes. The ranking metric measures a gene’s correlation with a phenotype. The value of the ranking metric goes from positive to negative as you move down the ranked list. A positive value indicates correlation with the first phenotype and a negative value indicates correlation with the second phenotype..\n\n", sep=""))
      ...
      for ( j in 1:nrow(top_gseKEGG.df[[i]]) ) {
        ...
        png(file = paste(KEGG_dir, "/summary_", rownames(top_gseKEGG.df[[i]])[j], ".gsea.png", sep=""))
        print(gseaplot2(top_gseKEGG[[i]], geneSetID = j, title = top_gseKEGG[[i]]$Description[1]))
        invisible(dev.off())
        
        cat(paste0("![](", paste(KEGG_dir, "/summary_", rownames(top_gseKEGG.df[[i]])[j], ".gsea.png)", sep="")), "\n")
      ...
      for ( j in 1:nrow(top_gseKEGG.df[[i]]) ) {
        ...
        png(file = paste(KEGG_dir, "/summary_", rownames(top_gseKEGG.df[[i]])[j], ".network.png", sep=""))
        print(cnetplot(topPathways.genes, foldChange=data.pathways[[i]]))
        invisible(dev.off())
        
        cat(paste0("![](", paste(KEGG_dir, "/summary_", rownames(top_gseKEGG.df[[i]])[j], ".network.png)", sep="")), "\n")
...
##### Create folder for storing pathway analysis results plots
DisGeNET_dir <- paste(params$output_dir, "KEGG_pathways", sep = "/")
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      if ( !is.null(enrichDGN.results) && nrow(enrichDGN.results) > 0 ) {
        ...
        for ( j in 1:nrow(enrichDGN.genes[[i]]) ) {
          ...
          write.table(data2plot_topGenes , file=paste0(DisGeNET_dir, "/enrichDGN_", colnames(eb$p.value)[i], "_", make.names(top_enrichKEGG.genes[[i]]$Description[j]), "_DE_genes_exprs.txt"),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
          
          write.table(topGenes_list_log2FC[ rownames(topGenes_list_log2FC) %in% topGenes_list.pathway, ] , file=paste0(DisGeNET_dir, "/enrichDGN_", colnames(eb$p.value)[i], "_", make.names(top_enrichKEGG.genes[[i]]$Description[j]), "_DE_genes_log2FC.txt"),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
          
          write.table(topGenes_list_p[ rownames(topGenes_list_log2FC) %in% topGenes_list.pathway, ] , file=paste0(DisGeNET_dir, "/enrichDGN_", colnames(eb$p.value)[i], "_", make.names(top_enrichKEGG.genes[[i]]$Description[j]), "_DE_genes_p_value.txt"),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
        }
...
`r if ( length(contrastNames) > 1 ) { c("Upset plot showing intersections of DisGeNET pathways over-representated across multiple comparisons.") }`
  ...
  if ( length(topGenes_list[[i]]) > 1 ) {
    ...
    if ( !is.na(enrichDGN[[i]]) ) {
      ...
      for ( j in 1:nrow(enrichDGN.df[[i]]) ) {
        ...
        png(file = paste(DisGeNET_dir, "/summary_", rownames(enrichDGN.df[[i]])[j], ".network.png", sep=""))
        print(cnetplot(topPathways.genes, foldChange=data.pathways[[i]]))
        invisible(dev.off())
        
        cat(paste0("![](", paste(DisGeNET_dir, "/summary_", rownames(enrichDGN.df[[i]])[j], ".network.png)", sep="")), "\n")
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      if ( !is.null(gseNCG.results) && nrow(gseNCG.results) > 0 ) {
        ...
        for ( j in 1:nrow(top_gseNCG.genes[[i]]) ) {
          ...
          write.table(data2plot_topGenes , file=paste0(DisGeNET_dir, "/gseNCG_", colnames(eb$p.value)[i], "_", make.names(top_enrichKEGG.genes[[i]]$Description[j]), "_DE_genes_exprs.txt"),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
          
          write.table(topGenes_list_log2FC[ rownames(topGenes_list_log2FC) %in% topGenes_list.pathway, ] , file=paste0(DisGeNET_dir, "/gseNCG_", colnames(eb$p.value)[i], "_", make.names(top_enrichKEGG.genes[[i]]$Description[j]), "_DE_genes_log2FC.txt"),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
          
          write.table(topGenes_list_p[ rownames(topGenes_list_log2FC) %in% topGenes_list.pathway, ] , file=paste0(DisGeNET_dir, "/gseNCG_", colnames(eb$p.value)[i], "_", make.names(top_enrichKEGG.genes[[i]]$Description[j]), "_DE_genes_p_value.txt"),sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
        }
...
`r if ( length(contrastNames) > 1 ) { c("Upset plot showing intersections of DisGeNET pathways enriched across multiple comparisons.") }`
  ...
  if ( length(topGenes_list[[i]]) > 1 ) {
    ...
    if ( !is.na(top_gseNCG[[i]]) ) {
      ...
      for ( j in 1:nrow(top_gseNCG.df[[i]]) ) {
        ...
        png(file = paste(DisGeNET_dir, "/summary_", rownames(top_gseNCG.df[[i]])[j], ".gsea.png", sep=""))
        print(gseaplot2(top_gseNCG[[i]], geneSetID = j, title = top_gseNCG[[i]]$Description[1]))
        invisible(dev.off())
        
        cat(paste0("![](", paste(DisGeNET_dir, "/summary_", rownames(top_gseNCG.df[[i]])[j], ".gsea.png)", sep="")), "\n")
      ...
      for ( j in 1:nrow(top_gseNCG.df[[i]]) ) {
        ...
        png(file = paste(DisGeNET_dir, "/summary_", rownames(top_gseNCG.df[[i]])[j], ".network.png", sep=""))
        print(cnetplot(topPathways.genes, foldChange=data.pathways[[i]]))
        invisible(dev.off())
        
        cat(paste0("![](", paste(DisGeNET_dir, "/summary_", rownames(top_gseNCG.df[[i]])[j], ".network.png)", sep="")), "\n")
