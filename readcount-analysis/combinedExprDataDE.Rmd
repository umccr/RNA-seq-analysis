...
for (i in 1:ncol(eb$p.value) ) {
  ...
  if ( !is.na(topGenes_list[[i]]) ) {
    ...
    if ( length(genes.ENTREZID) > 1 ) {
      ...
      enrichKEGG.results <- enrichKEGG(gene=genes.ENTREZID, organism = 'hsa', pvalueCutoff = params$pThreshold, use_internal_data = FALSE)
      
      if ( !is.null(enrichKEGG.results) && nrow(enrichKEGG.results) > 0  ) {
        
        topPathways_list[[i]] <- enrichKEGG.results
  ...
  if ( !is.na(topGenes_list[[i]]) ) {
    ...
    if ( !is.na(topPathways_list[[i]]) ) {
      ...
      for ( j in 1:nrow(topPathways_list[[i]]) ) {
        ...
        pathview.res <- pathview(gene.data  = data.pathways,
                     pathway.id = topPathways_list[[i]][j]$ID,
                     species    = "hsa",
                     limit      = list(gene=max(abs(data.pathways)), cpd=1),
                     kegg.dir = KEGG_dir)
        
        cat(paste0("![](", paste(KEGG_dir, "/", topPathways_list[[i]][j]$ID, ".pathview.png)", sep="")), "\n")
        cat("\n***\n")
      }
    } else {
