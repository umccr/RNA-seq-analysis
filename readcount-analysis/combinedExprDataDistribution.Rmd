...
##### Generate scatter plots for samples (highlighting the samples and genes of interest) to illustrate the effect of data normalization and z-score transformation. Currently, this function works for one gene at a time.
...
Box-plot below present z-score transformed data (`r if ( tolower(params$scaling) == "sample-wise" ) { c("sample-wise") } else { c("gene-wise") }`) for individual samples, coloured according to corresponding groups.
...
`r if ( tolower(params$norm) != "none" ) { c(paste0("Scatter-plots illustrating the effect of data normalisation and z-score transformation (", z_scaling, ") on the ", params$transform, " data. The ", params$transform, " and z-score data, respectively, are plotted as a function of ", paste0("normalised ", params$transform, " (", params$norm, ")"), " values.")) } else { c(paste0("Scatter-plots illustrating the effect of data z-score transformation (", z_scaling, ") on the ", params$transform, " data. The z-scores are plotted as a function of ", params$transform, " values.")) }`
...
if ( !is.na(gene) ) {
  ...
  if ( tolower(params$norm) != "none" ) {
    ...
    ##### Check the effect of data Z-score transformation 
    widges.list[[plotsNo+1]] <- comparePlot(genes=gene, dataset1=y.sub$norm, dataset2=y.sub$z, main_title="", x_title=paste0("Normalised ", params$transform, " (", params$norm, ")"), y_title="Z-score", sampleName=samples )
      
  ##### For normalisation OFF
  } else {
    
    ##### Check the effect of data Z-score transformation 
    widges.list[[plotsNo]] <- comparePlot(genes=gene, dataset1=transformed.data, dataset2=y.sub$z, main_title="Z-score transformation", x_title=params$transform, y_title="Z-score", sampleName=samples )
  }
}
...
`r if ( tolower(params$norm) != "none" ) { c(paste0("Plot(s) illustrating ", params$transform, ", normalised ", params$transform, " (", params$norm, ") and z-score transformated  (", z_scaling, ") data distributions for selected gene(s)")) } else { c(paste0("Plot(s) illustrating ", params$transform, " and z-score transformated (", z_scaling, ") data distributions for selected gene(s)")) }` along with simulated normal, binomial (p=0.25 and p=0.75) and bimodal distributions (computed based on the input data).
