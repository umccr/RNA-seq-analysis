...
params:
  ...
  output_dir: '/Library/WebServer/Documents/PIEdb/piedb_backoffice/data/expr_data_integration/datasets_prep/TCGA-PAAD_dataset'
  transform: 'TPM'
  norm: 'quantile'
  batch_rm: 'combat'
  batch_par: TRUE
  ...
  results_name: 'TCGA-PAAD_dataset'
  scaling: 'gene-wise'
  ref_data_dir: '../data'
  grch_version: 38
  ensembl_version: 86
...
input_dir <- paste(head(unlist(strsplit(params$datasets, split='/', fixed=TRUE)),-1), collapse = "/")

##### Extract info about target file for the first dataset
fileInfo = strsplit(DatasetInput[,"Target_file"], split='/', fixed=TRUE)
targetFile <- read.table(paste(input_dir, DatasetInput[1,"Target_file"], sep="/"), sep="\t", as.is=TRUE, header=TRUE)
...
if ( nrow(DatasetInput) > 1 ) {
  for ( i in 2:nrow(DatasetInput) ) {
    ...
    targetFile.list[[i]] <- read.table(paste(input_dir, DatasetInput[i,"Target_file"], sep="/"), sep="\t", as.is=TRUE, header=TRUE)
...
for ( i in 1:nrow(DatasetInput) ) {
  ...
  if (!exists("datasets")) {
    datasets <- as.data.frame( read.table(gzfile(paste(input_dir, DatasetInput[ i, "Expression_matrix" ], sep="/")), header=TRUE, sep="\t", row.names=NULL) )
    ...
    dataset <-as.data.frame( read.table(gzfile(paste(input_dir, DatasetInput[ i, "Expression_matrix" ], sep="/")), header=TRUE, sep="\t", row.names=NULL) )
...
if ( length(gene_list.missing) > 0 ) {
  write.table(prepare2write(gene_list.missing), file = paste0(params$output_dir, "/", params$results_name,".missing_genes.txt"), sep="\t", quote=FALSE, row.names=TRUE, append = FALSE )
}


##### Write combined count data into a file
write.table(prepare2write(datasets), file = paste0(params$output_dir, "/", params$results_name,".combined.counts.txt"), sep="\t", quote=FALSE, col.names=TRUE, row.names=FALSE, append = FALSE )
...
saveWidgetFix(p, file = paste0(params$output_dir, "/", params$results_name,".RNAseq_libSize.html"))
...
saveWidgetFix(p, file = paste0(params$output_dir, "/", params$results_name,".counts_vs_transformed.html"))
...
if ( params$filter ) {
  ...
  pdf(paste0(params$output_dir, "/", params$results_name,".filtering.pdf"), width=8, height=5, pointsize = 8)
  par(mfrow=c(1,2))
...
##### Save the plot as pdf file
pdf(paste0(params$output_dir, "/", params$results_name,".normalisation.pdf"), width=8, height=14, pointsize = 6)
...
if ( length(lib_size_plot) == 1 ) {
  ...
  ##### Save the bar-plot as html (PLOTLY)
  saveWidgetFix(p, file = paste0(params$output_dir, "/", paste0(params$results_name, ".library_size_norm.html")), selfcontained = TRUE)
}
```

```{r batch_effect_correction, comment = NA, message=FALSE, warning=FALSE, echo = FALSE}
...
if ( length(unique(batches)) > 1 ) {
  if ( params$batch_rm == "limma" ) {
  ...
  } else if ( params$batch_rm == "combat" ) {

    ##### Adjusting data for batch effects using ComBat. As default, parametric adjustments are used
    #####  Create model matrix for outcome of interest and other covariates besides batch
    f.model <- factor(targetFile$Target, levels = levels(factor(targetFile$Target)))
    ...
    for(i in 1:length(batches)) {
      batchenv[[ batches[i] ]] <- c(batchenv[[ batches[i] ]],f.model[i])
    }
    ...
    ##### Adjusting data for batch effects using ComBat
    if ( params$batch_par ) {
      ...
      png(paste0(params$output_dir, "/", params$results_name,".Combat_PriorPlot.png"), pointsize = 12, bg = "transparent")
      datasets.list[[dataset]][["batch_effect_corrected"]] = ComBat(dat=as.matrix(datasets.list[[dataset]][["combined_data_processed"]]), batch=batches, mod=mod, par.prior=params$batch_par, prior.plots=TRUE)
      dev.off()
    } else {
      datasets.list[[dataset]][["batch_effect_corrected"]] = ComBat(dat=as.matrix(datasets.list[[dataset]][["combined_data_processed"]]), batch=batches, mod=mod, par.prior=params$batch_par, prior.plots=FALSE)
    }
...
write.table(prepare2write(datasets.list[[dataset]][["batch_effect_corrected"]]), file = paste0(params$output_dir, "/", paste0(params$results_name, ".combined.txt")), sep="\t", quote=FALSE, row.names=FALSE, append = FALSE )

##### Write list genes into a file
write.table(prepare2write(rownames(datasets.list[[dataset]][["batch_effect_corrected"]])), file = paste0(params$output_dir, "/", params$results_name,".combined_genes.txt"), sep="\t", quote=FALSE, col.names=FALSE, row.names=FALSE, append = FALSE )
```

`r if ( batch_rm_status ) { c("***") }`

`r if ( batch_rm_status && params$batch_rm == "limma" ) { c("## Batch-effects correction") }`

`r if ( batch_rm_status && params$batch_rm == "combat" ) { c("## Batch-effects correction {.tabset}") }`

`r if ( batch_rm_status && params$batch_rm == "combat" ) { c("### PCA") }`
...
`r if ( batch_rm_status && params$batch_rm == "combat" ) { c("***") }`

`r if ( batch_rm_status && params$batch_rm == "combat" ) { c("### Prior plot") }`

`r if ( batch_rm_status && params$batch_rm == "combat" ) { c("Set of plots helping to assess whether parametric or nonparametric adjustments should be used. If the red (parametric estimate of the empirical batch effect density) and black (kernel estimate of the empirical batch effect density) lines do not match up well then nonparametric ajustments should be used.") }`


```{r prior_plot_batch_effect, warning = FALSE, echo = TRUE, message = FALSE, results = 'asis', eval=batch_rm_status, fig.width = 12, fig.height = 10}
##### Generate prior plots to assess if parametric or nonparametric adjustments should be used in ComBat. If the red (parametric estimate of the empirical batch effect density) and black (kernel estimate of the empirical batch effect density) lines don't match up well, use the nonparametric ajustments
if ( params$batch_rm == "combat" && params$batch_par ) {
  cat("![](",paste0(params$output_dir, "/", params$results_name,".Combat_PriorPlot.png"), ")", sep = "")
}
```
...
write.table(prepare2write(y[["comb"]]$z), file = paste0(params$output_dir, "/", paste0(params$results_name, ".combined.z.txt")), sep="\t", quote=FALSE, row.names=FALSE, append = FALSE )

##### Write list genes into a file
write.table(prepare2write(rownames(y[["comb"]]$z)), file = paste0(params$output_dir, "/", params$results_name,".combined.z_genes.txt"), sep="\t", quote=FALSE, col.names=FALSE, row.names=FALSE, append = FALSE )
```
...
##### Save the plot as pdf file
pdf(paste0(params$output_dir, "/", paste0(params$results_name, ".", scaled_title, ".pdf")), width=8, height=6, pointsize = 6)
par(mfrow=c(1,1), mar=c(2, 5, 3, 2))
...
col.h = colorRamp2(seq(min(data.h), max(data.h), length = 3), c("blue", "#EEEEEE", "red"))
  
Heatmap(data.h, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h)
  
pdf(paste0(params$output_dir, "/", paste0(params$results_name, ".heatmap.pdf")), width = 8, height = 8)
Heatmap(data.h, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h, use_raster = FALSE)
dev.off()
```
...
saveWidgetFix(p, file = paste0(params$output_dir, "/", paste0(params$results_name, ".heatmap.html")), selfcontained = TRUE)
```
...
pdf(paste0(params$output_dir, "/", paste0(params$results_name, ".heatmap_scaled.pdf")), width = 8, height = 8)
Heatmap(data.h, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h, use_raster = FALSE)
dev.off()
```
...
##### Save the heatmap as html (PLOTLY)
saveWidgetFix(p, file = paste0(params$output_dir, "/", paste0(params$results_name, ".heatmap_scaled.html")), selfcontained = TRUE)
...
if ( length(genes) > 0 ) {
    write.table(prepare2write(genes), file = paste0(params$output_dir, "/", paste0(params$results_name, ".genes.txt")), sep="\t", quote=FALSE, row.names=TRUE, append = FALSE )
...
if ( exists("genes.missing") && length(genes.missing) > 0 ) {
  write.table(prepare2write(genes.missing), file = paste0(params$output_dir, "/", paste0(params$results_name, ".genes.missing.txt")), sep="\t", quote=FALSE, row.names=TRUE, append = FALSE )
}
```
