...
densityPlot <- function(genes, data, main_title, x_title, sampleName, distributions = NULL, plot_mode = "static") {
  ...
  if ( !is.null(distributions) && length(genes) == 1) {
    ...
    if ("bimodal" %in% tolower(distributions)){
      ...
      #data.x <- c( min(expr.sorted), rtruncnorm(length(expr.sorted)/2 -1, a=min(expr.sorted), b=max(expr.sorted), mean=(min(expr.sorted) + (max(expr.sorted)/2))/2, sd=1),
      #            rtruncnorm(length(expr.sorted)/2 -1, a=min(expr.sorted), b=max(expr.sorted), mean=((max(expr.sorted)/2) + max(expr.sorted))/2, sd=1), max(expr.sorted))
      
      #data.y <- c(dtruncnorm(data.x[1:(length(data.x)/2)], a=min(data.x), b=max(data.x), mean = (min(expr.sorted) + (max(expr.sorted)/2))/2, sd = 1),
      #            dtruncnorm(data.x[((length(data.x)/2)+1):length(data.x)], a=min(data.x), b=max(data.x), mean=((max(expr.sorted)/2) + max(expr.sorted))/2, 
      #                       sd =1))
...
if ( !is.na(gene) ) {
  if ( params$filter ) {
    ...
    densityPlot(genes=gene, data=y.sub$filtered.transformed, main_title="Transformed and filtered data", x_title=params$transform,
                sampleName=samples, distributions = c("normal", "binomial",  "bimodal") )
  } else {
    densityPlot(genes=gene, data=y.sub$transformed, main_title="Transformed data", x_title=params$transform, sampleName=samples,
  ...
  if ( tolower(params$norm) != "none" ) {
    ...
    densityPlot(genes=gene, data=y.sub$norm, main_title="Normalised data", x_title=params$transform, sampleName=samples, 
                distributions = c("normal", "binomial", "bimodal") ) 
  }
  densityPlot(genes=gene, data=y.sub$z, main_title="Z-score data", x_title=params$transform, sampleName=samples,
              distributions = c("normal", "binomial",  "bimodal") )
}
```
