...
output:
  ...
  rmdformats::material:
    highlight: kate
params:
  datasets: '/Library/WebServer/Documents/PIEdb/piedb_backoffice/data/expr_data_integration/datasets_prep/TCGA-PAAD_dataset2/TCGA_PAAD_datasets_list.txt'
  report_dir: '/Library/WebServer/Documents/PIEdb/piedb_backoffice/data/expr_data_integration/datasets_prep/TCGA-PAAD_dataset2'
  transform: 'CPM'
  norm: 'TMM'
  batch_rm: TRUE
  filter: TRUE
...
densityPlot <- function(genes, data, main_title, x_title, sampleName, distributions = NULL) {
  ...
  if ( !is.null(distributions) && length(genes) == 1 ) {
    ...
    if ( "bimodal" %in% tolower(distributions) ){
      ...
      data.x1 <- seq(min(expr.sorted), median(expr.sorted), length.out = round(ncol(data)/2, digits=0))
      data.x2 <- seq(median(expr.sorted), max(expr.sorted), length.out = ceiling(ncol(data)/2))
...
if ( nrow(DatasetInput) > 1 ) {
  for ( i in 2:nrow(DatasetInput) ) {
    ...
    ##### Create a temporary object to store info from the remaining target files
    targetFileTmp <- read.table(paste(params$report_dir, DatasetInput[i,"Target_file"], sep="/"), sep="\t", as.is=TRUE, header=TRUE)
    
    ##### Make sure that there are no duplciated samples in the target file
    targetFileTmp <- targetFileTmp[!duplicated(targetFileTmp[,"Sample_name"]),]
    rownames(targetFileTmp) <- targetFileTmp[,"Sample_name"]
    targetFileTmp <- cbind(targetFileTmp[,"Target", drop = FALSE],rownames(DatasetInput[i,]))
    colnames(targetFileTmp)[ncol(targetFileTmp)] <- "Dataset"
...
for (data_matrix in DatasetInput[ , "Expression_matrix" ] ) {
  ...
  if (!exists("datasets")) {
    datasets <- as.data.frame( read.table(gzfile(paste(params$report_dir, data_matrix, sep="/")), header=TRUE, sep="\t", row.names=NULL) )
    ...
    dataset <-as.data.frame( read.table(gzfile(paste(params$report_dir, data_matrix, sep="/")), header=TRUE, sep="\t", row.names=NULL) )
...
for ( subset in targets_mod.list ) {
  ...
  if ( params$filter && params$transform == "CPM" ) {
    ...
    keep <- rowSums(cpm(y[[subset]])>1) >= ncol(data)/10
  ...
  ##### CPM transformation without filtering
    ...
    ##### Create EdgeR DGEList object
    y[[subset]] <- edgeR::DGEList(counts=data,  group=targetFile$Dataset)
  ...
  ##### TPM transformation with filtering
    ...
    keep <- rowSums(y[[subset]]$transformed>1+0.25) >= ncol(y[[subset]]$transformed)/10
    
    ##### Keep the genes of interest too
    keep[ names(keep) %in% genes ] <- TRUE
    y[[subset]]$filtered <- y[[subset]]$counts[keep, ]
...
if ( params$filter ) {
  ...
  if ( length(targets_mod.list) == 2 ) {
  
    genes_mod <- intersect(rownames(y[[targets_mod.list[1]]]$filtered$counts), rownames(y[[targets_mod.list[2]]]$filtered))
    ...
    y[["comb"]]$filtered <- y[[targets_mod.list[1]]]$filtered
...
##### Combine DGEList objects created for each dataset
...
write.table(prepare2write(y[["comb"]]$z), file = paste0(params$report_dir, "/", paste0(params$results_name, ".combined.txt")), sep="\t", quote=FALSE, row.names=FALSE, append = FALSE )

##### Write list genes into a file
write.table(prepare2write(rownames(y[["comb"]]$z)), file = paste0(params$report_dir, "/", params$results_name,".combined_genes.txt"), sep="\t", quote=FALSE, col.names=FALSE, row.names=FALSE, append = FALSE )
```
...
Principal component analysis (PCA) transforms the data into a coordinate system and presents them in an orthogonal projection. This reduces the dimensionality of the data and facilitates exploration of their global structure, as well as the key *components* of variation of the data.
...
```{r pca_no_batch_effect, comment = NA, message=FALSE, warning=FALSE}
suppressMessages(library(plotly))
...
pca(data=y[["comb"]]$z, targets=target)[[2]]
...
##### Subset data
y.sub <- y[["comb"]]
datasets.list[[dataset]][["combined_data_sub"]] <- datasets.list[[dataset]][["combined_data"]]
