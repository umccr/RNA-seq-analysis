---
title: "Investigate expression data"
author: "UMCCR"
date: "`r format(Sys.time(), '%d %B, %Y')`"
params:
  dataset: 'pdac'
  exprDir: '/Users/kanwals/Documents/UMCCR/data/RNAseq-report/ExpressionData'
...
combineDatasets <- function(ref_dataset) {
  ...
  ##### Read read count file and combine it with remaining reference datasets
  datasets.comb <- as.data.frame( read.table(DatasetInput[ , "Expression_matrix" ][1], header=TRUE, sep="\t", row.names=NULL) )
  
  ##### Add prexit to sample names
  colnames(datasets.comb) <- paste(levels(targetFile[,"Dataset"])[1], colnames(datasets.comb), sep = ".")
...
comparePlot <- function(genes, dataset1, dataset2, main_title, x_title, y_title, sampleName, plot_mode = "static") {
  ...
  for (i in 1:nrow(dataset1)) {
      data.df <- rbind(data.df, data.frame( gene = genes[i], sample = names(dataset1[ genes[i], ]), dataset1 = dataset1[ genes[i], ], dataset2 = dataset2[ genes[i], ]))
  }
...
densityPlot <- function(genes, data, main_title, x_title, sampleName, distributions = NULL, plot_mode = "static") {
  ...
  for (i in 1:nrow(data)) {
      data.df <- rbind(data.df, data.frame(gene = genes[i], sample = names(data[ genes[i], ]), expr = sort(data[ genes[i], ]), dens = density2freq(density(data[i,], n=ncol(data))$y)))
  }
...
ref_dataset <- list( "pdac" = c(paste(params$ref_data_dir, "Datasets_list_PDAC.txt", sep="/"), "PDAC (UMCCR)", "PDAC (TCGA)"), "cervix" = c(paste(params$ref_data_dir, "Datasets_list_cervical_SCC.txt", sep="/"), "PDAC (UMCCR)", "CESC (TCGA)"), "bladder" = c(paste(params$ref_data_dir, "Datasets_list_bladder.txt", sep="/"), "PDAC (UMCCR)", "BLCA (TCGA)") )
...
##### Read in reference datasets and merge them with UMCCR data. This part outputs a vector with first element containing the merged data and second element containing merged targets info
ref_datasets.list[[dataset]] <- combineDatasets(ref_dataset=ref_dataset[[dataset]][1])
...
targetFile <- ref_datasets.list[[dataset]][["sample_annot"]]

#### Check the number of datasets for pca plots
runPCAchunk <- FALSE

if ( length(unique(targetFile$Dataset)) > 1 ) {
...
for ( dataset in names(ref_datasets.list) ) {
  ...
  #data <- ref_datasets.list[[dataset]][["combined_data_processed"]]
  ...
  ref_datasets.list[[dataset]][["combined_data"]] <- apply(data.annot[, colnames(data)], 2, as.numeric)
  rownames(ref_datasets.list[[dataset]][["combined_data"]]) <- data.annot$SYMBOL
  ref_datasets.list[[dataset]][["gene_annot"]] <- data.annot[, c("SYMBOL", "GENEBIOTYPE", "ENSEMBL", "SEQNAME", "GENESEQSTART", "GENESEQEND")]
}
...
for ( group in targets_mod.list ) {
  ...
  if ( params$filter && params$transform == "CPM" ) {
    ...
    keep <- rowSums(cpm(y[[group]])>1) >= ncol(data)/10
  ...
  ##### CPM transformation without filtering
    ...
    ##### Create EdgeR DGEList object
    y[[group]] <- edgeR::DGEList(counts=data,  group=targetFile$Target)
...
for ( group in targets_mod.list ) {
  if ( params$transform == "CPM" ) {
    ...
    if ( params$filter ) {
      ...
      y[[group]]$samples["norm.factors"] <- edgeR::calcNormFactors(y, method = params$norm)$samples["norm.factors"]
...
## `r if ( length(unique(targetFile$Dataset)) > 1) { c("Batch-effect Correction") }`

```{r pca_batch_effect, comment = NA, message=FALSE, warning=FALSE}
suppressMessages(library(plotly))
...
for ( dataset in names(ref_datasets.list) ) {
  ...
  ref_datasets.list[[dataset]][["pca_combined_data_processed"]] <- pca(ref_datasets.list[[dataset]][["combined_data_processed"]], target)
  ref_datasets.list[[dataset]][["pca_batch_effect_corrected"]] <- pca(ref_datasets.list[[dataset]][["batch_effect_corrected"]], target)
}
...
if (tolower(params$scaling) == "gene-wise"){
  ...
  z_scaling <- "gene-wise"
  y[["comb"]]$z <- t(scale(t(ref_datasets.list[[dataset]][["batch_effect_corrected"]])))
} else {
  
  z_scaling <- "sample-wise"
  y[["comb"]]$z <- scale(ref_datasets.list[[dataset]][["batch_effect_corrected"]], scale = FALSE)
  }
...
```{r data_z_score_plots, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 12}
##### Plot expression distribution of samples for unnormalised and normalised data
par(mfrow=c(1,1), mar=c((max(nchar(colnames(y[["comb"]]$z)))+3)/2, 5, 3, 2))
    
boxplot(y[["comb"]]$z, las=2, col=targets.colour[[2]], main="", pch=".", las=3, xaxt = "n")
  title(main=paste0(scaled_title, " data"), ylab=scaled_title)
legend("topright", legend=levels(factor(targetFile$Target)), fill=targets.colour[[1]], horiz=TRUE, bg = "transparent", box.col="transparent")
    
##### Save the plot as pdf file
pdf(paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_", scaled_title, ".pdf")), width=8, height=7)
par(mfrow=c(1,1), mar=c(max(nchar(colnames(y[["comb"]]$z)))/2, 5, 3, 2))
...
`r if ( tolower(params$norm) != "none" ) { c(paste0("Scatter-plots illustrating the effect of data normalisation and ", if ( tolower(params$scaling) == "gene-wise" ) { c("z-score transformation") } else { c("centering") }, " data (", z_scaling, ") on the ", params$transform, " data. The ", params$transform, " and ", if ( tolower(params$scaling) == "gene-wise" ) { c("z-score data") } else { c(paste0("centered ", params$transform, " values")) }, " , respectively, are plotted as a function of ", paste0("normalised ", params$transform, " (", params$norm, ")"), " values.")) } else { c(paste0("Scatter-plots illustrating the effect of data ", if ( tolower(params$scaling) == "gene-wise" ) { c("z-score transformation") } else { c("centering") }, " (", z_scaling, ") on the ", params$transform, " data. The ", if ( tolower(params$scaling) == "gene-wise" ) { c("z-scores") } else { c(paste0("centered ", params$transform, "s")) }, " are plotted as a function of ", params$transform, " values.")) }`
...
if ( !is.na(gene) ) {
 ...
 ##### For normalisation ON
  if ( tolower(params$norm) != "none" ) {
    ...
    widges.list[[plotsNo]] <- comparePlot(genes=gene, dataset1=y[["comb"]]$norm, dataset2=transformed.data, main_title="", x_title=paste0("Normalised ", params$transform, " (", params$norm, ")"), y_title=params$transform, sampleName=samples )
    
    ##### Check the effect of data batch-effect correction
    widges.list[[plotsNo+1]] <- comparePlot(genes=gene, dataset1=ref_datasets.list[[dataset]][["batch_effect_corrected"]], dataset2=transformed.data, main_title="", x_title=paste0("Batch-effects corrected ", params$transform), y_title=paste0("Normalised ", params$transform, " (", params$norm, ")"), sampleName=samples )
    
    
    ##### Check the effect of data Z-score transformation/centering
    widges.list[[plotsNo+2]] <- comparePlot(genes=gene, dataset1=y[["comb"]]$z, dataset2=ref_datasets.list[[dataset]][["batch_effect_corrected"]], main_title="", x_title=scaled_title, y_title=paste0("Batch-effects corrected ", params$transform), sampleName=samples )
      
  ##### For normalisation OFF
  } else {
    
    ##### Check the effect of data batch-effect correction
    widges.list[[plotsNo]] <- comparePlot(genes=gene, dataset1=ref_datasets.list[[dataset]][["batch_effect_corrected"]], dataset2=transformed.data, main_title="",  x_title=paste0("Batch-effects corrected ", params$transform), y_title=paste0("Normalised ", params$transform, " (", params$norm, ")"), sampleName=samples )
...
`r if ( tolower(params$norm) != "none" ) { c(paste0("Plot(s) illustrating ", params$transform, ", normalised ", params$transform, " (", params$norm, ") and ", if ( tolower(params$scaling) == "gene-wise" ) { c("z-score transformed") } else { c("centered") }, " (", z_scaling, ") data distributions for selected gene(s)")) } else { c(paste0("Plot(s) illustrating ", params$transform, " and ", if ( tolower(params$scaling) == "gene-wise" ) { c("z-score transformed") } else { c("centered") }, " (", z_scaling, ") data distributions for selected gene(s)")) }` along with simulated normal, binomial (p=0.25 and p=0.75) and bimodal distributions (computed based on the input data).
