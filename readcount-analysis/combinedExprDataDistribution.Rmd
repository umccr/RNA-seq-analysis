...
params:
  ...
  genes: 'MKI67'
  ensembl: TRUE
  sample: 'CCR170012_MH17T001P013,CCR180029_MH18T002P038_RNA'
  results_name: 'test'
  ensembl_version: 86
output:
  html_document:
...
densityPlot <- function(genes, data, main_title, x_title, sampleName, distributions = NULL, plot_mode = "static") {
  ...
  ##### Extract positon for samples of interest from the data
  sampleName.position <- NULL
  for (sample in sampleName) {
    sample.intermediate <- NULL
    for ( gene in genes ) {
      sample.intermediate <- cbind(sample.intermediate, match(sample, names(sort(data[ gene, ]))))
    }
    sampleName.position <- rbind(sampleName.position, sample.intermediate)
  }
  
  #names(sample.inter) <- genes
  rownames(sampleName.position) = sampleName
  colnames(sampleName.position) = genes
  ...
  for (i in 1:nrow(data)) {
      data.df <- rbind(data.df, data.frame(gene = genes[i], sample = names(sort(data[ genes[i], ])), expr = density(data[i,], n= ncol(data))$x, dens = density2freq(density(data[i,], n=ncol(data))$y)))
  }
  
  expr.den <- density(data[i,], n= ncol(data))$x
  
  ##### Extract expression for selected sample
  #data.df.selected <- data.df[ data.df$sample %in% sampleName, ]
  ...
  if ( !is.null(distributions) && length(genes) == 1) {
    ...
    data.x <- seq(min(expr.den), max(expr.den), length.out = ncol(data))
    ...
    if ( "binomial" %in% tolower(distributions) ) {
      ...
      data.x <- rescale(data.x, to = c(min(expr.den), max(expr.den)))
    ...
    if ("bimodal" %in% tolower(distributions)){
      ...
      #expr.den.rescaled <- rescale(expr.den, to = c(-1.5, 19))
      
      #data.x <- c( min(expr.den), rtruncnorm(length(expr.den)/2 -1, a=min(expr.den), b=max(expr.den), mean=(min(expr.den*5) + (max(expr.den*5)/2))/2, sd=1),
      #            rtruncnorm(length(expr.den)/2 -1, a=min(expr.den), b=max(expr.den), mean=((max(expr.den*5)/2) + max(expr.den*5))/2, sd=1), max(expr.den))
      data.x <- c( min(expr.den), rtruncnorm(length(expr.den)/2 -1, a=min(expr.den), b=max(expr.den), mean=(min(expr.den) + (max(expr.den)/2))/2, sd=1),
                  rtruncnorm(length(expr.den)/2 -1, a=min(expr.den), b=max(expr.den), mean=((max(expr.den)/2) + max(expr.den))/2, sd=1), max(expr.den))
      
      #data.y <- c(dtruncnorm(data.x[1:(length(data.x)/2)], a=min(data.x), b=max(data.x), mean = (min(expr.den*5) + (max(expr.den*5)/2))/2, sd = 1),
      #            dtruncnorm(data.x[((length(data.x)/2)+1):length(data.x)], a=min(data.x), b=max(data.x), mean=((max(expr.den*5)/2) + max(expr.den*5))/2, 
      #                       sd =1))
      data.y <- c(dtruncnorm(data.x[1:(length(data.x)/2)], a=min(data.x), b=max(data.x), mean = (min(expr.den) + (max(expr.den)/2))/2, sd = 1),
                  dtruncnorm(data.x[((length(data.x)/2)+1):length(data.x)], a=min(data.x), b=max(data.x), mean=((max(expr.den)/2) + max(expr.den))/2, 
                             sd =1))
      data.y[c(1, length(data.y))] <- 0
      
      data.df.dist <- rbind(data.df.dist, data.frame(gene="Bimodal distribution", sample = names(sort(data[ genes, ])), expr=sort(data.x),
                                                           dens=density2freq(data.y[order(data.x)])*2))
    }
    
    data.df <- rbind(data.df, data.df.dist)
    ##### Extract expression for selected sample in the distributions dataframe
    #data.df.selected <- data.df[ strsplit(sampleName, split=',', fixed=TRUE) %in% data.df$sample, ]
    for ( i in 1:length(sampleName) ) {
      data.df.selected <- data.df[sampleName[i] == data.df$sample, ]
    }
  ...
  plotly::layout(title = main_title,
         ...
         yaxis = list (title = 'Frequency (smoothed)', range = c(den.y[1],den.y[length(den.y)]))) %>%
    
    plotly::add_annotations(y = data.df.selected$dens, x = data.df.selected$expr,
...
sample <- unique(unlist(strsplit(params$sample, split=',', fixed=TRUE)))

##### Change the target file to mark the sample of intersted
targetFile$Target[match(params$sample, targetFile$Sample_name)] <- "Sample"
...
**`r if ( !runChunk ) { c("Queried genes are not present in the input data.") } else { c("") }`**

```{r expr_dist_plot, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 5*(norm.height+1), eval = runChunk}
##### Plot expression distribution in transfromed and normalised data for selected genes
if ( params$filter ) {
  
  densityPlot(genes=genes, data=y.sub$filtered.transformed, main_title="Transformed and filtered data", x_title=params$transform,
              sampleName=sample, distributions = c("normal", "binomial",  "bimodal") )
  
} else {
  
  densityPlot(genes=genes, data=y.sub$transformed, main_title="Transformed data", x_title=params$transform, sampleName=sample,
...
if ( tolower(params$norm) != "none" ) {
  ...
  #### Interactive genes distribution plot
  densityPlot(genes=genes, data=y.sub$norm, main_title="Normalised data", x_title=params$transform, sampleName=sample, 
              distributions = c("normal", "binomial", "bimodal") ) 
}

densityPlot(genes=genes, data=y.sub$z, main_title="Z-score data", x_title=params$transform, sampleName=sample,
            distributions = c("normal", "binomial",  "bimodal") ) 
```
...
if ( params$filter ) {
 ...
 if ( nrow(y.sub$filtered.transformed) < 25 ) {
    
    DT::datatable( data = round(t(y.sub$filtered.transformed), digits = 1), filter="none", rownames = TRUE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800, caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left; color:grey; font-size:100% ;'), escape = FALSE) %>%
      DT::formatStyle( columns = names(t(y.sub$filtered.transformed)), `font-size` = '12px', 'text-align' = 'center' )
    
  } else {
    cat(paste0(nrow(y.sub$filtered.transformed), " genes were selected. The tables is available only if less than 25 genes are selected!"))
  }
...
if ( nrow(y.sub$z) < 25 ) {
  ...
  DT::datatable( data = round(t(y.sub$z), digits = 1), filter="none", rownames = TRUE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800, caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left; color:grey; font-size:100% ;'), escape = FALSE) %>%
      DT::formatStyle( columns = names(t(y.sub$z)), `font-size` = '12px', 'text-align' = 'center' )
    
  } else {
  cat(paste0(nrow(y.sub$z), " genes were selected. The tables is available only if less than 25 genes are selected!"))
}
```
