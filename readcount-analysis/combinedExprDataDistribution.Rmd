...
```{r distribution_simulations, comment = NA, message=FALSE, warning=FALSE, eval = runChunk}
##### Generate values to generate various distributions
##### Get min and max values based on the expression data
transformed.x <- seq(min(y$transformed), max(y$transformed), 0.1)

##### Generate y-values for normal distribution
transformed.y.norm <- dnorm(transformed.x, mean = mean(transformed.x), sd = 0.5)

##### Generate y-values for binomial distribution
#transformed.y.binom <- dbinom(transformed.x, size = 1000, prob = 0.25)

##### Create dataframe with similated values
data.df.dist <- data.frame(gene="Normal distribution", expr=transformed.x, dens=transformed.y.norm)
#data.df.dist <- rbind(data.df.dist, data.frame(gene="Gene_binom_dist", expr=transformed.x, dens=transformed.y.binom))
```
...
##### Create empty table
data.trans.df <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(data.trans.df) <- c("gene", "expr", "dens")

for (i in 1:nrow(y$transformed)) {
  
  data.trans.df <- rbind(data.trans.df, data.frame(gene = rownames(y$count)[i], expr = density(y$transformed[i,])$x, dens = density(y$transformed[i,])$y))
}

##### Add simulated distribution values
data.trans.df <- rbind(data.trans.df, data.df.dist)


plotly::plot_ly(data.trans.df, x = ~expr, y = ~dens, type = 'scatter', mode = 'lines', color = ~gene, colors = genes.colour, width = 1000, height = 300) %>%
  plotly::layout(title = 'Transformed data',
         xaxis = list(title = params$transform, range = c(den.x[1],den.x[length(den.x)])),
...
if ( tolower(params$norm) != "none" ) {
  ...
  #### Interactive genes distribution plot
  ##### Create empty table
  data.norm.df <- data.frame(matrix(ncol = 3, nrow = 0))
  colnames(data.norm.df) <- c("gene", "expr", "dens")
  
  for (i in 1:nrow(y$norm)) {
  
    data.norm.df <- rbind(data.norm.df, data.frame(gene = rownames(y$count)[i], expr = density(y$norm[i,])$x, dens = density(y$norm[i,])$y))
  }
  
  plotly::plot_ly(data.norm.df, x = ~expr, y = ~dens, type = 'scatter', mode = 'lines', color = ~gene, colors = genes.colour, width = 1000, height = 300) %>%
  plotly::layout(title = 'Normalised data',
         xaxis = list(title = params$transform, range = c(den.x[1],den.x[length(den.x)])),
         yaxis = list (title = 'Density', range = c(den.y[1],den.y[length(den.y)])))
}
...
legend("topright", legend=rownames(y$transformed), fill=genes.colour, bty="n", bg = "transparent")
...
if ( tolower(params$norm) != "none" ) {
  ...
  legend("topright", legend=rownames(y$norm), fill=genes.colour, bty="n", bg = "transparent")
} 
invisible(dev.off())
```
