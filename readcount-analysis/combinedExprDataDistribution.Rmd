...
if ( params$norm != "None" && params$norm != "none" ) {
  ...
  title(main=paste0("Normalised data (", tolower(params$norm), ")"), ylab=params$transform)
  legend("topright", legend=levels(factor(targetFile$Target)), fill=targets.colour[[1]], horiz=TRUE, bg = "transparent", box.col="transparent")
}
...
```{r expr_distribution_plot, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 6}
...
##### Transformed data
plot(density(y$transformed[1,]), lwd=2, ylim=c(0,0.4), las=2, main="", xlab="", col=genes.colour[1])
title(main="Transformed data", xlab=params$transform)
abline(v=0, lty=3)
...
if ( tolower(params$norm) != "none" ) {
  ...
  ##### Normalised data
  plot(density(y$norm[1,]), lwd=2, ylim=c(0,0.4), las=2, main="", xlab="", col=genes.colour[1])
  title(main="Transformed data", xlab=params$transform)
  abline(v=0, lty=3)
...
##### Save the plot as pdf file
pdf(paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_expr_distributions.pdf")), width=8, height=5)
if ( tolower(params$norm) != "none" ) {
  par(mfrow=c(1,2))
} else {
  par(mfrow=c(1,1))
...
```{r expr_distribution_table, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 6}
##### Generate a table with expression values for selected genes
if ( nrow(data) < 1000 ) {
  
  DT::datatable( data = t(data), filter="none", rownames = TRUE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800, caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left; color:grey; font-size:100% ;'), escape = FALSE) %>%
    DT::formatStyle( columns = names(t(data)), `font-size` = '12px', 'text-align' = 'center' )
  
} else {
  cat(paste0(nrow(data), " genes were selected. The tables is available only if less than 1000 genes are selected!"))
}
```
