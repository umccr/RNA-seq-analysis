...
for ( subset in targets_mod.list ) {
  ...
  ##### TPM transformation without filtering
    ...
    if ( params$norm == "sizeFactors" ) {
      y[[subset]]$norm <- edgeR::rpkm(y[[subset]], gene.length = gene.length, normalized.lib.sizes=TRUE, log=FALSE)
    }
...
pca(data=y[["comb"]]$norm, targets=target, dim=2)[[2]]
...
pca(data=y[["comb"]]$norm, targets=target, dim=3)[[2]]
