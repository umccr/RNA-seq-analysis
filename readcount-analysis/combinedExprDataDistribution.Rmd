...
for ( group in targets_mod.list ) {
  ...
  if ( params$filter && params$transform == "CPM" ) {
    ...
    if (runPCAchunk){
      y$notBatchCorrected <- y$filtered.transformed
      y$BatchCorrected <- limma::removeBatchEffect(y$filtered.transformed, batch = ref_datasets.list[[dataset]][["sample_annot"]]$Dataset)
      y$filtered.transformed <- y$BatchCorrected
    }
  ...
  ##### CPM transformation without filtering
    ...
    ##### 
    if (runPCAchunk){
      y$notBatchCorrected <- y$transformed
      y$BatchCorrected <- limma::removeBatchEffect(y$transformed, batch = ref_datasets.list[[dataset]][["sample_annot"]]$Dataset)
      y$transformed <- y$BatchCorrected
    }
...
##### Unnormalised data
boxplot(y[["comb"]]$noNorm, las=2, col=targets.colour[[2]], main="", pch=".", las=3)
...
if ( tolower(params$norm) != "none" ) {
  ...
  boxplot(y[["comb"]]$norm, las=2, col=targets.colour[[2]], main="", pch=".", las=3)
