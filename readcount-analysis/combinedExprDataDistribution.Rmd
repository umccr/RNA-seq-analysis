...
colnames(data.df) <- c("gene", "tpm", "dens")

for (i in 1:nrow(y$transformed)) {
  
  data.df <- rbind(data.df, data.frame(gene = rownames(y$count)[i],tpm = density(y$transformed[i,])$x, dens = density(y$transformed[i,])$y))
}
  
plotly::plot_ly(data.df, x = ~tpm, y = ~dens, type = 'scatter', mode = 'lines', color = ~gene, colors = genes.colour, width = 1000, height = 400) %>%
