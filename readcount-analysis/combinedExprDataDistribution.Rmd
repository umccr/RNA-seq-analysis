...
tpm.df <- data.frame(gene = NA, tpm = NA, dens = NA)
for (i in 1:nrow(y$transformed)) {
tpm <- data.frame(gene = rownames(y$count)[i],tpm = density(y$transformed[i,])$x, dens = density(y$transformed[i,])$y)
tpm.df <- rbind(tpm.df, tpm)
}
