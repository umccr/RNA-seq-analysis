...
densityPlot <- function(genes, data, main_title, x_title, sampleName, distributions = NULL, plot_mode = "static") {
  ...
  if ( !is.null(distributions) && length(genes) == 1) {
    ...
    if ("bimodal" %in% tolower(distributions)){
      data.x <- c(rtruncnorm(ncol(data)/2, a=min(expr.den), b=max(expr.den), mean=(min(expr.den) + (max(expr.den)/2))/2, sd=1),
                  rtruncnorm(ncol(data)/2, a=min(expr.den), b=max(expr.den), mean=((max(expr.den)/2) + max(expr.den))/2, sd=1))
      data.y <- c(dtruncnorm(data.x[1:(length(data.x)/2)], a=min(data.x), b=max(data.x), mean = mean(data.x[1:(length(data.x)/2)]), sd = 1),
                  dtruncnorm(data.x[((length(data.x)/2)+1):length(data.x)], a=min(data.x), b=max(data.x), mean =
                               mean(data.x[((length(data.x)/2)+1):length(data.x)]), sd = 1))
