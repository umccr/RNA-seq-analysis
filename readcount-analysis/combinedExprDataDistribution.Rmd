...
params:
  exprDir: '/Users/jmarzec/data/Pancreatic_data_harmonization/expression/in-house/UMCCR/Combined_data'
  ...
  results_name: 'expression_distributions_CPM_TMM'
  ensembl_version: 86
output:
  html_document:
    keep_md: yes
...
densityPlot <- function(genes, data, main_title, x_title, sampleName, distributions = NULL, plot_mode = "static") {
  ...
  if ( !is.null(distributions) && length(genes) == 1) {
    ...
    if ( "binomial" %in% tolower(distributions) ) {
      ...
      data.df.dist <- rbind(data.df.dist, data.frame(gene="Binomial distribution", sample = names(sort(data[ genes, ])), expr=data.x, dens=density2freq(data.y)))
...
`r if ( tolower(params$norm) != "none" ) { c(paste0("Scatter-plots illustrating the effect of data normalisation and z-score transformation on the ", params$transform, " data. The ", params$transform, " and z-score data, respectively, are plotted as a function of ", paste0("normalised ", params$transform), " values.")) } else { c(paste0("Scatter-plots illustrating the effect of data z-score transformation on the ", params$transform, " data. The z-scores are plotted as a function of ", params$transform, " values.")) }`
...
if ( params$filter ) {
  ...
  genes.missing <- genes[ genes %!in% rownames(y$filtered) ]
  genes <- genes[ genes %in% rownames(y$filtered)  ]
    
  keep <- rownames(y$filtered) %in% genes
...
if ( !is.na(gene) ) {
  ...
  if ( tolower(params$norm) != "none" ) {
    ...
    widges.list[[plotsNo]] <- comparPlot(genes=gene, dataset1=y.sub$norm, dataset2=transformed.data, main_title="Transformed vs normalised data", x_title=paste0("Normalised ", params$transform), y_title=params$transform, sampleName=samples )
      
    ##### Check the effect of data Z-score transformation 
    widges.list[[plotsNo+1]] <- comparPlot(genes=gene, dataset1=y.sub$norm, dataset2=y.sub$z, main_title="Normalised vs z-score data", x_title=paste0("Normalised ", params$transform), y_title="Z-score", sampleName=samples )
  ...
  ##### For normalisation OFF
    ...
    if ( params$filter ) {
      ...
      widges.list[[plotsNo]] <- comparPlot(genes=gene, dataset1=y.sub$filtered.transformed, dataset2=y.sub$z, main_title="Transformed vs z-score data", x_title=params$transform, y_title="Z-score", sampleName=samples )
      
    } else {
      
      widges.list[[plotsNo]] <- comparPlot(genes=gene, dataset1=y.sub$transformed, dataset2=y.sub$z, main_title="Transformed vs z-score data", x_title=params$transform, y_title="Z-score", sampleName=samples )
    }
  }
}
...
if ( !is.na(gene) ) {
  ...
  if ( tolower(params$norm) != "none" ) {
    ...
    if ( params$filter ) {
      ...
      widges.list[[plotsNo]] <- comparPlot(genes=gene, dataset1=y.sub$norm, dataset2=y.sub$filtered.transformed, main_title="Transformed vs normalised data", x_title="Normalised data", y_title=params$transform, sampleName=samples )
      
    } else {
      
      ##### Check the effect of data Z-score transformation 
      widges.list[[plotsNo]] <- comparPlot(genes=gene, dataset1=y.sub$norm, dataset2=y.sub$transformed, main_title="Transformed vs normalised data", x_title="Normalised data", y_title=params$transform, sampleName=samples )
    }
          
    ##### Check the effect of data Z-score transformation 
    widges.list[[plotsNo+1]] <- comparPlot(genes=gene, dataset1=y.sub$norm, dataset2=y.sub$z, main_title="Normalised vs Z-score data", x_title="Normalised data", y_title="Z-score data", sampleName=samples )
  ...
  ##### For normalisation OFF
    ...
    if ( params$filter ) {
      ...
      widges.list[[plotsNo]] <- comparPlot(genes=gene, dataset1=y.sub$filtered.transformed, dataset2=y.sub$z, main_title="Transformed vs Z-score data", x_title=params$transform, y_title="Z-score data", sampleName=samples )
      
    } else {
      
      widges.list[[plotsNo]] <- comparPlot(genes=gene, dataset1=y.sub$transformed, dataset2=y.sub$z, main_title="Transformed vs Z-score data", x_title=params$transform, y_title="Z-score data", sampleName=samples )
    }
  }
}
...
Plot(s) presenting `r params$transform` data distribution for selected gene(s).
...
if ( !is.na(gene) ) {
  if ( params$filter ) {
    ...
    widges.list[[plotsNo]] <- densityPlot(genes=gene, data=y.sub$filtered.transformed, main_title="Transformed and filtered data", x_title=params$transform, sampleName=samples, distributions = c("normal", "binomial",  "bimodal") )
  } else {
    widges.list[[plotsNo]] <- densityPlot(genes=gene, data=y.sub$transformed, main_title="Transformed data", x_title=params$transform, sampleName=samples, distributions = c("normal", "binomial", "bimodal") ) 
  }
  ...
  if ( tolower(params$norm) != "none" ) {
    ...
    widges.list[[plotsNo]] <- densityPlot(genes=gene, data=y.sub$norm, main_title="Normalised data", x_title=params$transform, sampleName=samples, 
                distributions = c("normal", "binomial", "bimodal") ) 
  }
  widges.list[[plotsNo+1]] <- densityPlot(genes=gene, data=y.sub$z, main_title="Z-score data", x_title="Z-score", sampleName=samples,
              distributions = c("normal", "binomial",  "bimodal") )
}
...
if ( !is.na(gene) ) {
  ...
  if ( tolower(params$norm) != "none" ) {
    ...
    #### Interactive gene distribution plot
    widges.list[[plotsNo]] <- densityPlot(genes=gene, data=y.sub$norm, main_title="Normalised data", x_title=params$transform, sampleName=samples,distributions = c("normal", "binomial", "bimodal") ) 
  }
  widges.list[[plotsNo+1]] <- densityPlot(genes=gene, data=y.sub$z, main_title="Z-score data", x_title="Z-score", sampleName=samples, distributions = c("normal", "binomial",  "bimodal") )
}
