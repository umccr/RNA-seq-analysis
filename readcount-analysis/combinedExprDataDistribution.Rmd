...
densityPlot <- function(genes, data, main_title, x_title, sampleName, distributions = NULL, plot_mode = "static") {
  ...
  if ( !is.null(distributions) && length(genes) == 1) {
    ...
    if ("bimodal" %in% tolower(distributions)){
      ...
      data.x1 <- seq(min(expr.den), median(expr.den), length.out = ncol(data)/2)
      data.x2 <- seq(median(expr.den), max(expr.den), length.out = ncol(data)/2)
      ...
      data.y <- c(dnorm(data.x1, mean = mean(data.x1), sd = (max(data.x1)-mean(data.x1))/3),
                  dnorm(data.x2, mean = mean(data.x2), sd = (max(data.x2)-mean(data.x2))/3))
      
      ##### Add bimodal dist values to the distribution dataframe
      data.df.dist <- rbind(data.df.dist, data.frame(gene="Bimodal distribution", sample = names(sort(data[ genes, ])), expr=data.x,
                                                           dens=density2freq(data.y)))
    }
