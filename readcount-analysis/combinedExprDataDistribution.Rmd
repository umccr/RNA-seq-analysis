...
params:
  ...
  filter: TRUE
  log: TRUE
  genes: 'MKI67,KRAS,CDKN2A'
  ensembl: TRUE
  sample: 'CCR180029_MH18T002P038_RNA'
  results_name: 'test'
  ensembl_version: 86
output:
  html_document:
...
densityPlot <- function(genes, data, main_title, x_title, sampleName, distributions = NULL, plot_mode = "static") {
  ...
  for ( gene in genes ) {
    ...
    sampleName.position <- c(sampleName.position, match(sampleName, names(sort(data[ gene, ]))))
  }
  
  names(sampleName.position) <- genes
  ...
  if ( !is.null(distributions) ) {
    ...
    if ( "normal" %in% tolower(distributions) ) {
      ...
      data.df.dist <- rbind(data.df.dist, data.frame(gene="Normal distribution", expr=data.x, dens=density2freq(data.y)))
