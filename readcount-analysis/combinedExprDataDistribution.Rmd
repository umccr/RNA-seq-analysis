...
output:
  ...
  rmdformats::material:
    highlight: kate
params:
  datasets: '../data/test_data/test.datasets_list.txt'
  report_dir: '../data/test_data'
  transform: 'CPM'
  norm: 'TMM'
  filter: TRUE
  log: TRUE
...
if ( params$filter ) {
  ...
  pdf(paste0(params$report_dir, "/", params$results_name,".filtering.pdf"), width=8, height=5)
  par(mfrow=c(1,2))
...
##### Save the plot as pdf file
pdf(paste0(params$report_dir, "/", params$results_name,".normalisation.pdf"), width=8, height=14)
...
## `r if ( length(unique(targetFile$Dataset)) > 1) { c("Batch-effects correction") }`
...
datasets.list[[dataset]][["pca_combined_data_processed"]] <- pca(datasets.list[[dataset]][["combined_data_processed"]], target)
datasets.list[[dataset]][["pca_batch_effect_corrected"]] <- pca(datasets.list[[dataset]][["batch_effect_corrected"]], target)
...
```{r data_z_score_plot, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 8}
...
##### Save the plot as pdf file
pdf(paste0(params$report_dir, "/", paste0(params$results_name, ".", scaled_title, ".pdf")), width=8, height=6)
par(mfrow=c(1,1), mar=c(2, 5, 3, 2))
...
`r if ( runChunk ) { c("**Counts data**") } else { c("**Queried genes are not present in the input data!**") }`
...
`r if ( runChunk ) { c("**Transformed data**") } else { c(" ") }`
...
`r if ( tolower(params$norm) != "none" && runChunk ) { c("**Normalised data**") } else { c(" ") }`
...
`r if ( runChunk ) { c("**Z-score data**") } else { c(" ") }`
