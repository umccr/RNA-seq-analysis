...
params:
  exprDir: '/Users/jmarzec/data/Cohort_analyses/expression/CUP/Combined_data'
  exprFile: 'CUP.counts.matrix.txt'
  annotFile: 'CUP_Target.txt'
  transform: 'CPM'
  filter: TRUE
  norm: 'TMM'
  log: TRUE
...
#htmlwidgets::saveWidget(as_widget(p), paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, "_RNAseq_libSize.html")), selfcontained = TRUE)
...
##### Transformation to CPM, RPKM or TPM scale (see these blogs for details https://www.rna-seqblog.com/rpkm-fpkm-and-tpm-clearly-explained/ and https://haroldpimentel.wordpress.com/2014/05/08/what-the-fpkm-a-review-rna-seq-expression-units/ ).  CPM = Counts Per Million, RPKM = Reads Per Kilobase Million,  TPM = Transcripts Per Kilobase Million. 
...
if ( params$filter ) {
  ...
  legend("topright", legend=unique(targetFile$Target), fill=targets.colour[[1]], bty="n", bg = "transparent")
  ...
  pdf(paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, "_", params$transform, "_", params$norm, "_filtering.pdf")), width=8, height=5)
  par(mfrow=c(1,2))
...
```{r data_normalisation_unnorm_vs_norm, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 12}
...
if ( params$transform == "CPM" ) {
  ...
  if ( params$filter ) {
    ...
    y$noNorm <- edgeR::cpm(y$filtered, normalized.lib.sizes=TRUE, log=params$log, prior.count=0.25)
    y$norm <- edgeR::cpm(y$filtered, normalized.lib.sizes=TRUE, log=params$log, prior.count=0.25, method = "TMM")
  
  } else {
    
    y$noNorm <- edgeR::cpm(y, normalized.lib.sizes=TRUE, log=params$log, prior.count=0.25, method = "TMM")
    y$norm <- edgeR::cpm(y, normalized.lib.sizes=TRUE, log=params$log, prior.count=0.25, method = "TMM")
  }
...
##### Quantile normalsation (from https://www.biostars.org/p/296992/ )
  ...
  ##### Unnormalised data
  boxplot(y$noNorm, las=2, col=targets.colour[[2]], main="", pch=".", las=3, xaxt="n")
  title(main="Unnormalised data", ylab=params$transform)
  legend("topright", legend=unique(targetFile$Target), fill=targets.colour[[1]], horiz=TRUE, bg = "transparent", box.col="transparent")
  
  ##### Normalised data
  boxplot(y$norm, las=2, col=targets.colour[[2]], main="", pch=".", las=3, xaxt="n")
  title(main=paste0("Normalised data (", params$norm, ")"), ylab=params$transform)
  legend("topright", legend=unique(targetFile$Target), fill=targets.colour[[1]], horiz=TRUE, bg = "transparent", box.col="transparent")
  
  ##### Save the plot as pdf file
  pdf(paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, "_", params$transform, "_", params$norm, "_normalisation.pdf")), width=8, height=8)
  par(mfrow=c(2,1), mar=c(2, 5, 3, 2))
  ...
  # htmlwidgets::saveWidget(as_widget(p), paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, "_", params$transform, "_", params$norm, "_normalisation.html")), selfcontained = TRUE)
  # #######################################################
```
...
#htmlwidgets::saveWidget(as_widget(p), paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, "_PCA_2d.html")), selfcontained = TRUE)
...
##### Save PCA plot as html
#htmlwidgets::saveWidget(p, file = paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, "_PCA_3d.html")), selfcontained = TRUE)
```
...
#htmlwidgets::saveWidget(p, file = paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, "_PCA_pairwise.html")), selfcontained = TRUE)

##### Plot all combinations between the first 5 PCs
#pairs(y$norm.subset_pca$x[,1:5], pch=16, col=targets.colour[[2]])

##### Save the plot as pdf file
#pdf(paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, "_PCA_pairwise.pdf")), width=10, height=10)
#pairs(y$norm.subset_pca$x[,1:5], pch=16, col=targets.colour[[2]])
#invisible(dev.off())
```
...
#htmlwidgets::saveWidget(p, file = paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, "_scree_plot.html")), selfcontained = TRUE)
...
write.table(geneMatrix2write(y$norm), file=paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, "_", params$transform, "_", params$norm, "_", filtered, "_", logged, ".txt")), sep="\t", quote=FALSE, row.names=FALSE, append = FALSE)
```

The combined `r params$transform` expression data were saved in `r paste0(params$exprFile, "_", params$transform, "_", params$norm, "_", filtered, "_", logged, ".txt")`.
