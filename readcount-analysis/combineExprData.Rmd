...
#### Batch-effect correction using DESeq2. It will be useful to later look at https://rdrr.io/bioc/DESeq2/f/vignettes/DESeq2.Rmd to further analyse dds object
dds <- DESeqDataSetFromMatrix(countData = dataset, colData = targets, design = ~Target)
dds <- DESeq(dds)

#### Batch effect correction using RUVSeq

##### Identifying control genes. To estimate the factors of unwanted variation, we need a set of negative control genes, i.e., genes that can be assumed not to be influenced by the covariates of interest (in the case of the zebrafish dataset, the Gallein treatment). In many cases, such a set can be identified, e.g., housekeeping genes or spike-in controls. If a good set of negative controls is not readily available, one can define a set of “in-silico empirical” controls, as follows.
##### First, estimate the number of latent factors. num.sv function calculates the number of surrogate variables to estimate in a model
##### NOTE: we use "be" method since the "leek" methods outputs very high number that later throws an error when used in the empirical.controls function.

mod1 = model.matrix(~as.factor(Target), data = targets)
n.sv = num.sv(y.norm$counts, mod1, method="be")

##### Then, estimate the probability that each gene is an empirical control
control_genes_estimates <- empirical.controls(y.norm$counts, mod1, mod0 = NULL, n.sv, B = 5, type = "norm")

##### Use 0.99 from empirical.controls funtcion as a cut-off for defining control genes
control_genes <- rownames(y.norm$counts)[control_genes_estimates > 0.99]
