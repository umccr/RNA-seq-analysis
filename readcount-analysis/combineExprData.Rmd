...
#saveWidgetFix(as_widget(p), paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_RNAseq_libSize.html")))
...
##### TPM transformation without filtering
  ...
  if ( params$norm == "sizeFactors" ) {
    y$norm <- edgeR::rpkm(y, gene.length = gene.length, normalized.lib.sizes=TRUE, log=FALSE)
  }
...
# saveWidgetFix(as_widget(p), paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_", params$transform, "_", params$norm, "_normalisation.html")))
########################################################
```
...
if ( params$norm == "sizeFactors" ) {
  ...
  ##### Prepare data frame
  data.df <- data.frame(colnames(y$norm), as.numeric(colSums(y$norm)*1e-6))
...
if ( length(lib_size_plot) == 1 ) {
  ...
  ##### Save the bar-plot as html (PLOTLY)
  saveWidgetFix(p, file = paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_library_size_norm.html")), selfcontained = TRUE)
}
```
...
if ( nrow(y$norm.subset) < 300 ) {

    sel<-order(rsd, decreasing=TRUE)[1:nrow(y$norm.subset)]
} else {
    sel<-order(rsd, decreasing=TRUE)[1:300]
}
...
saveWidgetFix(as_widget(p), paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_PCA_2d.html")))
```
...
##### Save PCA plot as html
saveWidgetFix(p, file = paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_PCA_3d.html")))
```
...
saveWidgetFix(p, file = paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_PCA_pairwise.html")))
...
saveWidgetFix(p, file = paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_scree_plot.html")))
...
Heatmap presenting **300 genes** (*y-axis*) with the highest expression variance across all samples (*x-axis*).
...
##### Cluster genes
hc <- hclust(as.dist(1-cor(data, method="pearson")), method="ward.D2")

##### Cluster samples
hr <- hclust(as.dist(dist(data, method="euclidean")), method="ward.D2")
...
Heatmap(data, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split)
  
pdf(paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_heatmap.pdf")), width = 8, height = 8)
...
p <- heatmaply(data.frame(data), Rowv = hr, Colv = rev(hc), k_col=1, k_row=1, colors = colorRampPalette(c("darkblue","darkblue","darkslateblue","darkslateblue","white","firebrick3","firebrick3","firebrick4","firebrick4"))(100), col_side_colors = targetFile[, "Target"], scale="row", trace="none", hide_colorbar = TRUE, fontsize_row = 8, fontsize_col = 8) %>%
layout(autosize = TRUE, width = 800, margin = list(l=150, r=50, b=150, t=50, pad=4), showlegend = FALSE)

##### Save the heatmap as html (PLOTLY)
saveWidgetFix(p, file = paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_heatmap.html")), selfcontained = TRUE)
