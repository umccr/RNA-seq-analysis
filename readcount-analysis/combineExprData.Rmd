...
* **clust_samples**: `r params$clust_samples`
* **clust_genes**: `r params$clust_genes`
* **goi**: `r params$goi`
* **convert2gene_symbol**: `r params$convert2gene_symbol`
...
```{r gene_annot_count_data, comment = NA, message=FALSE, warning=FALSE, eval=params$convert2gene_symbol}
...
```{r biotype_detection, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 6, eval=params$convert2gene_symbol}
...
```{r count_per_biotype, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 6, eval=params$convert2gene_symbol}
##### Count distribution per biotype
...
```{r count_per_sample, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 6, eval=params$convert2gene_symbol}
...
```{r saturation, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 6, eval=params$convert2gene_symbol}
##### Count distribution per biotype
...
if ( ncol(data_NOISeq) > 10 ) {
  ...
  pdf(paste0(saturation_dir, "/saturation.pdf"))
  explo.plot(saturation, toplot = 1, samples = 1:ncol(data_NOISeq))
  invisible(dev.off())
}
...
```{r sensitivity, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 6, eval=params$convert2gene_symbol}
...
`r if ( runGOI ) { c("#### Most variable genes {.tabset .tabset-fade}") }`
...
`r if ( runGOI ) { c("##### 2D plot") } else { c("#### 2D plot") }`
...
`r if ( runGOI ) { c("##### 3D plot") } else { c("#### 3D plot") }`
...
`r if ( runGOI ) { c("##### Pariwise plot") } else { c("#### Pariwise plot") }`
...
`r if ( runGOI ) { c("##### Scree plot") } else { c("#### Scree plot") }`
...
```{r gene_annotation, comment = NA, message=FALSE, warning=FALSE, eval=params$convert2gene_symbol}
##### Convert data into a data frame to make the Ensembl ID and gene symbol matches (with merge function)
data <- y$norm
data.df <- as.data.frame(cbind(rownames(data), data))
colnames(data.df)[1] <- "ENSEMBL"
...
`r if ( runGOI ) { c("#### Genes of interest {.tabset .tabset-fade}") }`

`r if ( runGOI ) { c("Principal component analysis (PCA) transforms the data into a coordinate system and presents them in an orthogonal projection. This reduces the dimensionality of the data and facilitates exploration of their global structure, as well as the key *components* of variation of the data. The colours indicate sample groups. **Genes of interest** were used for PCA.") }`
...
`r if ( runGOI ) { c("Scatter plot of the first 2 principal components (PCs) constituting the primary source of variation in the data.") }`

```{r PCA_plot_2D_goi, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 8, eval = runGOI}
suppressMessages(library(plotly))

##### Keep only genes of interest
data_goi <- data.frame(data[ rownames(data) %in% goi_list,])

if ( length(targetFile$Target) > 2 ) {
  ##### Keep only genes with variance > 0 across all samples
  rsd <- apply(data_goi,1,sd)
  data_goi.subset <- data_goi[rsd>0,]
  rsd <- rsd[rsd>0]
  
  if ( nrow(data_goi.subset) < params$top_genes ) {
      sel<-order(rsd, decreasing=TRUE)[1:nrow(data_goi.subset)]
  } else {
      sel<-order(rsd, decreasing=TRUE)[1:params$top_genes]
  }
  
  ##### Subset genes
  data_goi.subset <- data.frame(data_goi.subset[sel,])

  ##### Perform PCA
  data_goi.subset_pca <- prcomp(t(data_goi.subset), scale=FALSE)
  
  ##### Get variance importance for all principal components
  importance_pca <- summary(data_goi.subset_pca)$importance[2,]
  importance_pca <- paste(round(100*importance_pca, 2), "%", sep="")
  names(importance_pca) <- names(summary(data_goi.subset_pca)$importance[2,])
    
  ##### Prepare data frame
  data_goi.subset_pca.df <- data.frame(targetFile$Target, data_goi.subset_pca$x[,"PC1"], data_goi.subset_pca$x[,"PC2"], data_goi.subset_pca$x[,"PC3"])
  colnames(data_goi.subset_pca.df) <- c("Target", "PC1", "PC2", "PC3")
  
  ##### Generate PCA 2-D plot
  p <- plot_ly(data_goi.subset_pca.df, x = ~PC1, y = ~PC2, color = ~Target, text=paste(targetFile$Target, rownames(data_goi.subset_pca.df), sep=": "), colors = targets.colour[[1]], type='scatter', mode = "markers", marker = list(size=10, opacity = 0.7), width = 600, height = 400) %>%
  layout(title = "", xaxis = list(title = paste( "PC1", " (",importance_pca["PC1"],")",sep="")), yaxis = list(title = paste( "PC2", " (",importance_pca["PC2"],")",sep="")), margin = list(l=50, r=50, b=50, t=20, pad=4), autosize = FALSE, showlegend = TRUE, legend = list(orientation = "v", y = 0.9))

  ##### Save PCA plot as html
  saveWidgetFix(as_widget(p), paste0(params$output_dir, "/", paste0(params$results_name, "_PCA_2d_goi.html")))
...
`r if ( runGOI ) { c("Scatter plot of the first 3 principal components (PCs) constituting the primary source of variation in the data.") }`

```{r PCA_plot_3D_goi, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 8, eval = runGOI}
##### Generate PCA 3-D plot
if ( length(targetFile$Target) > 2 ) {
  p <- plot_ly(data_goi.subset_pca.df, x = ~PC1, y = ~PC2, z = ~PC3, color = ~Target, text=paste(targetFile$Target, rownames(data_goi.subset_pca.df), sep=": "), colors = targets.colour[[1]], type='scatter3d', mode = "markers", marker = list(size=6, opacity = 0.7), width = 600, height = 600) %>%
  layout(title = "", scene = list(xaxis = list(title = paste( "PC1", " (",importance_pca["PC1"],")",sep="")), yaxis = list(title = paste( "PC2", " (",importance_pca["PC2"],")",sep="")), zaxis = list(title = paste( "PC3", " (",importance_pca["PC3"],")",sep=""))) , margin = list(l=50, r=50, b=50, t=10, pad=4), autosize = FALSE, showlegend = TRUE, legend = list(orientation = "v", y = 0.7))
  
  ##### Save PCA plot as html
  saveWidgetFix(p, file = paste0(params$output_dir, "/", paste0(params$results_name, "_PCA_3d_goi.html")))
...
`r if ( runGOI ) { c("Scatter plots for all combinations between the first 5 principal components (PCs) to help determine which PCs explain the biological and/or potential batch effects.") }`

```{r PCA_pairwise_plot_goi, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 8, eval = runGOI}
##### Plot all combinations between the first 5 PCs
##### Preapre colours for plot exis
axis = list(showline=FALSE, zeroline=FALSE, gridcolor='#ffff', ticklen=5)
...
if ( length(targetFile$Target) > 4 ) {
  ...
  p <- as.data.frame(data_goi.subset_pca$x[,1:5]) %>%
    plot_ly(width = 800, height = 800) %>%
    add_trace( type = 'splom', dimensions = list(
        list(label='PC1', values=~PC1), list(label='PC2', values=~PC2), list(label='PC3', values=~PC3), list(label='PC4', values=~PC4), list(label='PC5', values=~PC5)
      ),
    text=paste(targetFile$Target, rownames(data_goi.subset_pca$x), sep=": "),
      marker = list( color = targets.colour[[2]], size = 7, opacity = 0.7
      )
    ) %>%
    layout(
  ...
  ##### Save pairwise PCA plot as html
  saveWidgetFix(p, file = paste0(params$output_dir, "/", paste0(params$results_name, "_PCA_pairwise_goi.html")))
  
  p
  
  ##### Plot all combinations between the first 5 PCs
  #pairs(data_goi.subset_pca$x[,1:5], pch=16, col=targets.colour[[2]])
  ##### Save the plot as pdf file
  #pdf(paste0(params$output_dir, "/", paste0(params$results_name, "_PCA_pairwise_goi.pdf")), width=10, height=10)
  #pairs(data_goi.subset_pca$x[,1:5], pch=16, col=targets.colour[[2]])
  #invisible(dev.off())
}
```
...
`r if ( runGOI ) { c("Scree plot presenting the fraction of total variance (y-axis) attributed to each PC (x-axis). The PCs are ordered by decreasing order of contribution to total variance. ") }`

```{r Scree_plot_goi, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 8, eval = runGOI}
##### Generate scree plot
##### Prepare data frame
if ( length(targetFile$Target) > 2 ) {
  
  data_goi.subset_scree.df <- data.frame(paste0("PC ", c(1:length(importance_pca))), as.numeric(gsub("%", "",importance_pca)))
  colnames(data_goi.subset_scree.df) <- c("PC", "Variances")
  
  ##### The default order will be alphabetized unless specified as below
  data_goi.subset_scree.df$PC <- factor(data_goi.subset_scree.df$PC, levels = data_goi.subset_scree.df[["PC"]])
  p <- plot_ly(data_goi.subset_scree.df, x = ~PC, y = ~Variances, type = 'bar', width = 600, height = 400) %>%
    layout(title = "The variances captured by principal components", xaxis = list(title = ""), margin = list(l=50, r=50, b=100, t=100, pad=4), autosize = F)
  
  ##### Save scree plot as html
  saveWidgetFix(p, file = paste0(params$output_dir, "/", paste0(params$results_name, "_scree_plot_goi.html")))
...
##### Keep only genes of interest
data.h_goi <- data.frame(data[ rownames(data) %in% goi_list,])
...
##### Cluster samples
if ( params$clust_samples ) {
  hc <- hclust(as.dist(1-cor(data.h_goi, method="pearson")), method="ward.D2")
} else {
  hc <- FALSE
}
...
##### Cluster genes
if ( params$clust_genes ) {
  hr <- hclust(as.dist(dist(data.h_goi, method="euclidean")), method="ward.D2")
} else {
  hr <- FALSE
}
...
##### Scale rows the data first (row scale)
data.h_goi.z <- t(scale(t(data.h_goi)))
  
##### Define colour space ( https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html#colors )
col.h = colorRamp2(seq(min(data.h_goi.z), max(data.h_goi.z), length = 3), c("blue", "#EEEEEE", "red"))

Heatmap(data.h_goi.z, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h)

##### Set heatmaps height
if ( nrow(data.h_goi.z) < 10 ) {
  heatmap_goi_height <- 4
} else {
  heatmap_goi_height <- 8
}

pdf(paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_goi.pdf")), width = 12, height = heatmap_goi_height)
Heatmap(data.h_goi.z, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, col = col.h, use_raster = FALSE)
dev.off()

##### Return matrix with row/column sorting as in heatmap
##### Cluster samples
if ( params$clust_samples ) {
  data.h_goi.z.heatmap <- data.h_goi.z[, hc$labels[hc$order]]
} else {
  data.h_goi.z.heatmap <- data.h_goi.z
}

##### Cluster genes
if ( params$clust_genes ) {
  data.h_goi.z.heatmap <- data.h_goi.z.heatmap[hr$labels[hr$order], ]
}

##### Write data from the heatmap into a file
write.table(data.h_goi.z.heatmap, paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_goi_data.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
```

```{r heatmap_interactive_goi, warning = FALSE, echo = TRUE, message = FALSE, results = 'hide', fig.width = 12, fig.height = 10, eval = runGOI}
suppressMessages(library(heatmaply))

##### Generate heatmap (PLOTLY)
p <- heatmaply(data.frame(data.h_goi), Rowv = hr, Colv = rev(hc), k_col=1, k_row=1, colors = colorRampPalette(c("darkblue","darkblue","darkslateblue","darkslateblue","white","firebrick3","firebrick3","firebrick4","firebrick4"))(100), col_side_colors = col_side_colors, col_side_palette = col_side_palette, scale="row", trace="none", hide_colorbar = TRUE, fontsize_row = 8, fontsize_col = 8) %>%
layout(autosize = TRUE, width = 800, margin = list(l=150, r=50, b=150, t=50, pad=4), showlegend = FALSE)
...
```{r heatmap_batch_rm, warning = FALSE, echo = TRUE, message = TRUE, results = 'hide', fig.width = 12, fig.height = 10, eval=params$batch_rm}
##### Keep only genes with variance > 0 across all samples
...
if ( params$clust_samples ) {
  ...
  data.h_batch_rm.z.heatmap <- data.h_batch_rm.z
}
...
```{r write_data_final, comment = NA, message=FALSE, warning=FALSE}
...
##### And for GOIs
if ( runGOI ) {
  write.table(data_goi, file=paste0(params$output_dir, "/", paste0(params$results_name, "_", filtered, "_", logged, "_annot_goi.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
}
```
