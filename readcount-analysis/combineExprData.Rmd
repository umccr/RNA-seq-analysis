...
targetFile <- read.table(paste(params$projectDir,DatasetInput[1,"Target_file"],sep="/"), sep="\t", as.is=TRUE, header=TRUE)[,c(1:4)]
...
if ( nrow(DatasetInput) > 1 ) {
  for ( i in 2:nrow(DatasetInput) ) {
    ...
    targetFileTmp <- read.table(paste(params$projectDir,DatasetInput[i,"Target_file"],sep="/"), sep="\t", as.is=TRUE, header=TRUE)[,c(1:4)]
...
for (data_matrix in DatasetInput[ , "Expression_matrix" ] ) {
  ...
  if (!exists("datasets")) {
    datasets <- as.data.frame( read.table(paste(params$projectDir,data_matrix,sep="/"), header=TRUE, sep="\t", row.names=NULL) )
    ...
    dataset <-as.data.frame( read.table(paste(params$projectDir,data_matrix,sep="/"), header=TRUE, sep="\t", row.names=NULL) )
...
Remove user-defined outliers from individual datasets based on information in the outliers files.
...
for ( i in 1:length(datasetIDs) ) {
  ...
  if ( !is.empty(DatasetInput[ i, "Outliers_file"]) ) {
    ...
    FileInfo = file.info(paste(params$projectDir,DatasetInput[ i, "Outliers_file"],sep="/"))
    
    ##### If non-empty then read it in
    if ( FileInfo$size != 0 ) {
      
      outliersInfo = read.table( paste(params$projectDir,DatasetInput[ i, "Outliers_file"],sep="/"), sep="\t", as.is=TRUE, header=FALSE, row.names=NULL)
...
### Scaling factor (colour datasets)
