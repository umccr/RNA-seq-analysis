...
##### Cluster samples
if ( params$clust_samples ) {
  data.h.heatmap <- data.h[, hc$labels[hc$order]]
} else {
  data.h.heatmap <- data.h
}

##### Cluster genes
if ( params$clust_genes ) {
  data.h.heatmap <- data.h.heatmap[hr$labels[hr$order], ]
}

##### Write data from the heatmap into a file
write.table(data.h.heatmap, paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_data.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
```
...
##### Cluster samples
if ( params$clust_samples ) {
  data.h_goi.heatmap <- data.h_goi[, hc$labels[hc$order]]
} else {
  data.h_goi.heatmap <- data.h_goi
}

##### Cluster genes
if ( params$clust_genes ) {
  data.h_goi.heatmap <- data.h_goi.heatmap[hr$labels[hr$order], ]
}

##### Write data from the heatmap into a file
write.table(data.h_goi.heatmap, paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_goi_data.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
```
...
##### Cluster samples
if ( params$clust_samples ) {
  data.h_batch_rm.heatmap <- data.h_batch_rm[, hc$labels[hc$order]]
} else {
  data.h_batch_rm.heatmap <- data.h_batch_rm
}

##### Cluster genes
if ( params$clust_genes ) {
  data.h_batch_rm.heatmap <- data.h_batch_rm.heatmap[hr$labels[hr$order], ]
}

##### Write data from the heatmap into a file
write.table(data.h_batch_rm.heatmap, paste0(params$output_dir, "/", paste0(params$results_name, "_heatmap_batch_rm_data.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE )
