...
if ( !all( colnames(data) %in% rownames(targetFile) , na.rm = FALSE)  ) {
  ...
  write.table(data, file=paste0(normalizePath(params$exprDir), "/", paste0(params$exprFile, ".subset.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
} else {
  data <- data[ , rownames(targetFile) ]
}
...
if ( params$goi != "none" ) {
  goi_list <- unique(unlist(strsplit(params$goi, split=',', fixed=TRUE)))
  runGOI <- TRUE
} else {
  runGOI <- FALSE
}
...
saveWidgetFix(as_widget(p), paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_libSize.html")))
...
if ( length(lib_size_plot) == 1 ) {
  ...
  ##### Save the bar-plot as html (PLOTLY)
  saveWidgetFix(lib_size_plot[[1]], file = paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_library_size_norm.html")), selfcontained = TRUE)
}
```
...
if ( length(targetFile$Target) > 2 ) {
  ...
  cat("Only two samples were analysed!")
}
```
...
if ( length(targetFile$Target) > 4 ) {
  ...
  cat("Less than five samples were analysed!")
}
```
...
pdf(paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_heatmap_goi.pdf")), width = 8, height = 8)
Heatmap(data.h, name = "expr", top_annotation = ha, cluster_rows = hr, cluster_columns = hc, row_split = row_split, column_split = column_split, use_raster = FALSE)
dev.off()
```
...
##### Save the heatmap as html (PLOTLY)
saveWidgetFix(p, file = paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_heatmap_goi.html")), selfcontained = TRUE)
...
write.table(y$norm, file=paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_", filtered, "_", logged, ".txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)

##### Same for annotated genes
write.table(data, file=paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_", filtered, "_", logged, "_annot.txt")), sep="\t", quote=FALSE, row.names=TRUE, col.names=NA, append = FALSE)
```
