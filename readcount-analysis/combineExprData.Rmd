...
Before principal component analysis, we can perform some exploratory data analysis such as relative log expression (RLE) plot, correlation matrix and scatter plot matrix. This procedure follows [this tutorial](http://www.sthda.com/english/wiki/print.php?id=202) and implements fucntions from the [inSilicoMerging](http://www.bioconductor.org/packages//2.10/bioc/vignettes/inSilicoMerging/inst/doc/inSilicoMerging.pdf) package.
...
##### Generate scree plot
##### Prepare data frame
y.norm.cpm.subset_pca.df <- data.frame(paste0("PC ", c(1:length(y.norm.cpm.subset_pca$sdev))), y.norm.cpm.subset_pca$sdev)
colnames(y.norm.cpm.subset_pca.df) <- c("PC", "Variances")

##### The default order will be alphabetized unless specified as below
y.norm.cpm.subset_pca.df$PC <- factor(y.norm.cpm.subset_pca.df$PC, levels = y.norm.cpm.subset_pca.df[["PC"]])

##### Generate scree plot
p <- plot_ly(y.norm.cpm.subset_pca.df, x = ~PC, y = ~Variances, type = 'bar', width = 800, height = 600) %>%
...
##### Prepare data frame
y.norm.cpm.subset_pca.df <- data.frame(targetFile$Target, y.norm.cpm.subset_pca$x[,"PC1"], y.norm.cpm.subset_pca$x[,"PC2"], y.norm.cpm.subset_pca$x[,"PC3"])
colnames(y.norm.cpm.subset_pca.df) <- c("Target", "PC1", "PC2", "PC3")

##### Generate PCA 2-D plot
p <- plot_ly(y.norm.cpm.subset_pca.df, x = ~PC1, y = ~PC2, color = ~Target, text=rownames(y.norm.cpm.subset_pca.df), colors = targets.colour[[1]], type='scatter', mode = "markers", marker = list(size=10, opacity = 0.7, symbol=datasets.symbols[[2]][order(targetFile$Target)]), width = 800, height = 600) %>%
...
htmlwidgets::saveWidget(as_widget(p), paste0(normalizePath(params$projectDir), "/", paste(datasetIDs, collapse="_"),"_PCA_2d.html"), selfcontained = TRUE)

##### Generate PCA 3-D plot
p <- plot_ly(y.norm.cpm.subset_pca.df, x = ~PC1, y = ~PC2, z = ~PC3, color = ~Target, text=rownames(y.norm.cpm.subset_pca.df), colors = targets.colour[[1]], type='scatter3d', mode = "markers", marker = list(size=8, opacity = 0.7, symbol=datasets.symbols[[2]][order(targetFile$Target)]), width = 800, height = 800) %>%
...
##### Save PCA plot as html
htmlwidgets::saveWidget(as_widget(p), paste0(normalizePath(params$projectDir), "/", paste(datasetIDs, collapse="_"),"_PCA_3d.html"), selfcontained = TRUE)

##### Plot all combinations between the first 5 PCs (colour by datasets)
...
p <- as.data.frame(y.norm.cpm.subset_pca$x[,1:5]) %>%
  ...
  text=rownames(y.norm.cpm.subset_pca$x),
...
##### Plot all combinations between the first 5 PCs (colour by targets)
p <- as.data.frame(y.norm.cpm.subset_pca$x[,1:5]) %>%
  plot_ly(width = 800, height = 800) %>%
  add_trace( type = 'splom', dimensions = list(
      list(label='PC1', values=~PC1), list(label='PC2', values=~PC2), list(label='PC3', values=~PC3), list(label='PC4', values=~PC4), list(label='PC5', values=~PC5)
