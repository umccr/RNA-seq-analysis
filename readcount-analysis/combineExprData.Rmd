...
if ( params$filter && tolower(params$transform) == "cpm" ) {
...
} else if ( !params$filter && tolower(params$transform) == "cpm" ) {
...
} else if ( params$filter && tolower(params$transform) == "tpm" ) {
...
if ( tolower(params$transform) == "cpm" ) {
  ...
  if ( params$filter ) {
    ...
    y$filtered.transformed <- edgeR::calcNormFactors(y$filtered, method = tolower(params$norm))
...
} else if ( tolower(params$transform) == "tpm" ) {
...
write.table(geneMatrix2write(y$norm), file=paste0(normalizePath(params$exprDir), "/", paste0(params$results_name, "_", filtered, "_", logged, ".txt")), sep="\t", quote=FALSE, row.names=FALSE, append = FALSE)
```
