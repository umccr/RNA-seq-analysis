...
Heatmap presenting **300 genes** (*x-axis*) with the highest expression variance across all samples (*y-axis*).
...
##### Keep 300 genes with the highest expression variance across samples
rsd<-apply(data,1,sd)
...
p <- heatmaply(data.frame(data.h), Rowv = hr, Colv = rev(hc), k_col=1, k_row=1, colors = colorRampPalette(c("darkblue","darkblue","darkslateblue","darkslateblue","white","firebrick3","firebrick3","firebrick4","firebrick4"))(100), col_side_colors = targetFile[, "Target"], scale="row", trace="none", hide_colorbar = TRUE, fontsize_row = 8, fontsize_col = 8) %>%
